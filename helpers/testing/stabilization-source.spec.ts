...
describe('stabilization source', () => {
  ...
  it('keeps the generated-docs refresh checkpoint tied to current evidence', () => {
    ...
    expect(source).toContain('style-attribute CSS image URLs');
    expect(source).toContain('style-block CSS image declarations');
    ...
    expect(source).toContain('bun run test:e2e:reporter-paths');
    expect(source).toContain('31 passed (3.7s)');
    ...
    expect(source).toContain('style-attribute CSS image\nURLs');
    expect(source).toContain('style-block CSS image declarations');
    expect(source).toContain(
    ...
    expect(inventory).toContain('nested `.at(...)` screenshot-function');
    expect(inventory).toMatch(
    ...
    expect(inventory).toContain('nested `.at(...)` raw image payload objects');
    ...
    expect(generatedDocumentationSource).toContain(
      'groupedImageEvidence.payloads.at(0).raw',
    );
    expect(generatedDocumentationSource).toContain(
      'groupedImageEvidence.payloads.at(-1).raw',
    );
    expect(generatedDocumentationSource).toContain(
      'screenshotHelpers.list[0][0]',
    );
    expect(generatedDocumentationSource).toContain(
      'screenshotHelpers.list.at(0).at(0)',
    );
    expect(generatedDocumentationSource).toContain(
      'screenshotHelpers.list.at(-1).at(-1)',
    );
    expect(generatedDocumentationSource).toContain(
    ...
    expect(generatedDocumentationSource).toContain(
      'detects weak documentation screenshot targets preserved by concat receiver arrays',
    );
    expect(generatedDocumentationSource).toContain(
    ...
    expect(generatedDocumentationSource).toContain(
      'detects raw markdown images hidden behind forwarded markdown names',
    );
    expect(generatedDocumentationSource).toContain(
    ...
    expect(generatedDocumentationSource).toContain(
      'markdownEvidence.payloads.at(0).raw',
    );
    expect(generatedDocumentationSource).toContain(
      'markdownEvidence.payloads.at(-1).raw',
    );
    expect(generatedDocumentationSource).toContain(
    ...
    expect(inventory).toContain('style-attribute CSS image URLs');
    expect(inventory).toContain('style-block CSS\n  image declarations');
    ...
    expect(inventory).toContain('nested `.at(...)` screenshot-function');
    expect(inventory).toMatch(
      /negative\s+nested `\.at\(\.\.\.\)` screenshot-function/u,
    );
    ...
    expect(inventory).toContain('nested `.at(...)` raw image payload objects');
    expect(inventory).toContain(
      'negative nested `.at(...)` raw image payload objects',
    );
    ...
    expect(inventory).toContain('nested `.at(...)` raw Markdown payloads');
    expect(inventory).toMatch(
      /negative nested `\.at\(\.\.\.\)`\s+raw Markdown payloads/u,
    );
    expect(inventory).toContain(
  ...
  it('keeps the runtime status summary honest after the Font Awesome blocker moved', () => {
    ...
    expect(source).toMatch(/live\s+PR\s+checks\s+are tracked\s+separately/u);
  ...
  it('keeps the current Docker Browser runtime checkpoint tied to recovered Browser evidence', () => {
    ...
    expect(testInventory).toContain(
      'later source-only evidence-recording pushes preserved that\n  checkpoint until fresher Browser evidence was captured',
    );
    expect(testInventory).toMatch(
      /forced Docker\s+Desktop\s+backend\s+restart\s+recovered the local container start\s+path/u,
    );
    expect(testInventory).toContain(
  ...
  it('keeps the observed PR CI cleanup checkpoint tied to active-head evidence', () => {
    ...
    expect(checkpointText).toContain('local head `2667494b`');
    ...
    expect(checkpointText).toContain('E2E run `27068078693`');
    expect(checkpointText).toContain(
