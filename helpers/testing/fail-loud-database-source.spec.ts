...
describe('fail-loud local database and seed source', () => {
  it('routes every project-owned local Drizzle push through the guarded config', () => {
    ...
    const integrationEnvironment = source(
      'helpers/testing/postgres-integration-environment.ts',
    );
    ...
    expect(integrationEnvironment).toContain("DATABASE_TLS_REQUIRED: 'false'");
    expect(integrationEnvironment).toContain(
