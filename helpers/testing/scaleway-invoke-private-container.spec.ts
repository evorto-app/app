...
describe('Scaleway private-container invocation', () => {
  ...
  it('keeps the secret and request body out of curl arguments and environment', () => {
    const sensitiveValue = 'sensitive_process_listing_must_not_contain_this';
    const requestBody = JSON.stringify({ value: sensitiveValue });
    ...
    expect(invocation).not.toContain(sensitiveValue);
    expect(invocation).not.toContain(requestBody);
    expect(environment).toBeDefined();
    expect(environment).not.toContain(secretKey);
    expect(environment).not.toContain(sensitiveValue);
  ...
  it('refuses an operation outside the three bounded ops routes', () => {
    const { invocation, result } = invokeWithFakeCurl({
      operationPath: '/internal/ops/unexpected',
      responseBody: '{}',
      status: 200,
    });
  ...
  it('binds payment setup to the worker endpoint', () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toBe(JSON.stringify({ attached: true }));
    expect(result.stderr).toBe('');
    expect(invocation).toContain(
  ...
  it('refuses an ops operation on the worker endpoint before invoking curl', () => {
    const { invocation, result } = invokeWithFakeCurl({
      endpoint: trustedEndpoints.worker,
      operationPath: '/internal/ops/schema-explain',
      responseBody: '{}',
