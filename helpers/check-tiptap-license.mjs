...
for (const name of tiptapDeps) {
  ...
  try {
    ...
    console.error(`Unable to read installed package metadata for ${name} at ${path}`);
    process.exit(1);
  }

  const license = dependencyPackage.license;
  if (license !== 'MIT') {
    console.error(`Expected MIT license for ${name}, received: ${String(license)}`);
    process.exit(1);
  }
}

console.log('Tiptap dependency license check passed (MIT-only, OSS packages only).');
