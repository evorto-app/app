...
- Effect RPC `events.reviewEvent` + `events.submitForReview` + `events.updateListing` migration validation:
  ...
  - `CI=true bun run build`
  - `CI=true bun run test` (`12 passed`)
- Effect RPC `events.create` + `events.update` migration validation:
...
- Effect RPC `events.registerForEvent` + `events.cancelPendingRegistration` + `events.registrationScanned` migration validation:
  ...
  - `CI=true bun run build`
  - `CI=true bun run test` (`12 passed`)
- Effect RPC template simple-flow migration validation (`templates.findOne`, `templates.createSimpleTemplate`, `templates.updateSimpleTemplate`):
  - `CI=true bun run lint` (warnings-only baseline unchanged)
  - `CI=true bun run build`
  - `CI=true bun run test` (`12 passed`)
- tRPC namespace decommission validation (`users`, `globalAdmin` removed from app-router composition):
  - `CI=true bun run lint` (warnings-only baseline unchanged)
  - `CI=true bun run build`
  - `CI=true bun run test` (`12 passed`)
- Effect RPC finance migration validation (`finance.receiptMedia.uploadOriginal`, `finance.receipts.*`, `finance.transactions.findMany`):
  - `CI=true bun run lint` (warnings-only baseline unchanged: `45 warnings`, `0 errors`)
  - `CI=true bun run build`
  - `CI=true bun run test` (`12 passed`)
- tRPC finance namespace decommission validation (`finance` removed from app-router composition):
  - `CI=true bun run lint` (warnings-only baseline unchanged: `45 warnings`, `0 errors`)
  - `CI=true bun run build`
  - `CI=true bun run test` (`12 passed`)
