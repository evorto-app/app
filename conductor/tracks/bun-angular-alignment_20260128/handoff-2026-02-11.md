# Session Handoff — 2026-02-11

## Scope Completed

Applied migration to `@heddendorp/effect-angular-query@0.1.1` across the active Effect RPC Angular integration path.
Also completed the follow-up cutover that removes the remaining imperative `EffectRpcClient` auth/config wrapper.
Also migrated `users.userAssigned` from tRPC to Effect RPC for the `user-account.guard` path.
Also migrated `users.maybeSelf`, `users.self`, and `users.updateProfile` from tRPC to Effect RPC across auth/events/profile flows.

### Code Changes

- Upgraded dependency:
  - `package.json`: `@heddendorp/effect-angular-query` -> `0.1.1`
  - `bun.lock` updated
- Added typed Angular client factory:
  - `src/app/core/effect-rpc-angular-client.ts`
  - uses `createEffectRpcAngularClient({ group, keyPrefix, queryDefaults, mutationDefaults, rpcLayer })`
- App provider migration:
  - `src/app/app.config.ts` now registers `AppRpc.providers`
  - removed old `provideEffectRpcQueryClient(...)` wiring
- Shared procedure markers introduced:
  - `src/shared/rpc-contracts/app-rpcs.ts`
  - procedures now explicitly wrapped with `asRpcQuery(...)` / `asRpcMutation(...)`
- Replaced old query client injection API:
  - removed remaining `EffectRpcQueryClient` + `helpersFor(...)` usage
  - switched to `AppRpc.injectClient()` in remaining callsites
- Mutation callsites migrated to generated helpers:
  - `injectMutation(() => rpc.<path>.mutationOptions())` now used for icons/templateCategories mutation flows
- Removed obsolete wrapper methods from imperative client:
  - `src/app/core/effect-rpc-client.ts`
  - removed migrated methods: `addIcon`, `searchIcons`, `createTemplateCategory`, `updateTemplateCategory`
  - follow-up completed: removed remaining guard/bootstrap methods and deleted the file entirely
- Added user-assigned guard procedure on Effect RPC and removed its tRPC equivalent:
  - `src/shared/rpc-contracts/app-rpcs.ts`: added `users.userAssigned` query contract
  - `src/server/effect/rpc/app-rpcs.web-handler.ts`: added `x-evorto-user-assigned` header bridge
  - `src/server/effect/rpc/app-rpcs.handlers.ts`: added `users.userAssigned` handler
  - `src/app/core/guards/user-account.guard.ts`: switched from `injectTRPCClient().users.userAssigned.query()` to `AppRpc.injectClient().users.userAssigned.call()`
  - `src/server/trpc/users/users.router.ts`: removed `userAssigned` procedure
- Added users session/profile procedures on Effect RPC and removed their tRPC equivalents:
  - `src/shared/rpc-contracts/app-rpcs.ts`: added `users.maybeSelf`, `users.self`, and `users.updateProfile` contracts
  - `src/server/effect/rpc/app-rpcs.web-handler.ts`: added `x-evorto-user` header bridge
  - `src/server/effect/rpc/app-rpcs.handlers.ts`: added handlers for `users.maybeSelf`, `users.self`, and `users.updateProfile`
  - Migrated Angular callsites in:
    - `src/app/core/auth.ts`
    - `src/app/events/event-list.service.ts`
    - `src/app/events/event-details/event-details.component.ts`
    - `src/app/events/guards/event-edit.guard.ts`
    - `src/app/profile/user-profile/user-profile.component.ts`
    - `src/app/core/create-account/create-account.component.ts`
  - `src/server/trpc/users/users.router.ts`: removed `maybeSelf`, `self`, and `updateProfile` procedures

## Validation Executed

### Passed

- `bunx --bun eslint` on all migrated files
- `bunx --bun tsc -p tsconfig.app.json --noEmit`
- `bunx --bun tsc -p tsconfig.spec.json --noEmit`
- `bash -lc 'eval "$(bun helpers/testing/runtime-env.mjs)" && CI=true NO_WEBSERVER=true bunx --bun playwright test tests/specs/templates/templates.test.ts --project=local-chrome --workers=1 --max-failures=1'`
  - result: `11 passed`
- `bash -lc 'eval "$(bun helpers/testing/runtime-env.mjs)" && CI=true NO_WEBSERVER=true bunx --bun playwright test tests/docs/templates/templates.doc.ts --project=docs --workers=1 --max-failures=1'`
  - result: `8 passed`
- `bunx --bun eslint src/app/core/guards/auth.guard.ts src/app/core/guards/user-account.guard.ts src/app/core/config.service.ts src/app/core/effect-rpc-angular-client.ts`
  - result: passed
- `bunx --bun tsc -p tsconfig.app.json --noEmit` and `bunx --bun tsc -p tsconfig.spec.json --noEmit`
  - result: passed
- `bash -lc 'eval "$(bun helpers/testing/runtime-env.mjs)" && CI=true NO_WEBSERVER=true bunx --bun playwright test tests/specs/templates/templates.test.ts --project=local-chrome --workers=1 --max-failures=1'`
  - result: `11 passed` after auth/config cutover
- `CI=true bun run lint`
  - result: passed (warnings-only baseline) after `users.userAssigned` Effect RPC cutover
- `CI=true bun run build`
  - result: passed after `users.userAssigned` Effect RPC cutover
- `CI=true bun run test`
  - result: `12 passed` after `users.userAssigned` Effect RPC cutover
- `CI=true bun run lint`
  - result: passed (warnings-only baseline) after `users.maybeSelf`/`users.self`/`users.updateProfile` cutover
- `CI=true bun run build`
  - result: passed after `users.maybeSelf`/`users.self`/`users.updateProfile` cutover
- `CI=true bun run test`
  - result: `12 passed` after `users.maybeSelf`/`users.self`/`users.updateProfile` cutover
- `bash -lc 'eval "$(bun helpers/testing/runtime-env.mjs)" && CI=true NO_WEBSERVER=true bunx --bun playwright test tests/docs/profile/discounts.doc.ts --project=docs --workers=1 --max-failures=1'`
  - result: `8 passed` after `users.maybeSelf`/`users.self`/`users.updateProfile` cutover

### Environment Status

- Earlier in the day this shell intermittently terminated `bunx --bun ng ...` with `SIGKILL`.
- Current re-validation in this same session succeeded:
  - `CI=true bun run lint` (warnings-only baseline)
  - `CI=true bun run build`
  - `CI=true bun run test` (`12 passed`)

## Remaining Opportunities / Next Steps

1. Expand Effect RPC vertical slices beyond templates/icons/users session/profile procedures (events/finance/admin/profile discounts paths still primarily tRPC).
2. Rerun full Bun gates (`lint:fix`, `lint`, `build`, `test`, `e2e`, `e2e:docs`) in one clean pass and append/update final verification snapshot.

## Conductor Artifacts Updated

- `conductor/tracks/bun-angular-alignment_20260128/plan.md`
  - added v0.1.1 migration task entry and latest validation notes
- `conductor/tracks/bun-angular-alignment_20260128/spec.md`
  - updated reference baseline file to v0.1.1 repomix
  - updated test mapping with current validation set
- `conductor/tracks/bun-angular-alignment_20260128/handoff-2026-02-11.md`
  - this document
