...
export const seedProfileEventCards = async ({
  ...
  const cleanup = async () => {
    ...
    for (const remove of [
      ...
      () =>
        database
          .delete(schema.eventRegistrations)
          .where(
            eq(schema.eventRegistrations.id, pendingCheckoutRegistrationId),
          ),
      () =>
        database
          .delete(schema.eventRegistrations)
      ...
      () =>
        database
          ...
          .where(
            eq(schema.eventRegistrationOptions.id, pendingCheckoutOptionId),
          ),
      () =>
        database
          .delete(schema.eventRegistrationOptions)
      ...
      () =>
        database
          ...
          .where(
            eq(
              schema.addonToEventRegistrationOptions.addonId,
              confirmedAddonId,
            ),
          ),
      () =>
      ...
      () =>
        database
          .delete(schema.eventRegistrationOptions)
          .where(
            eq(schema.eventRegistrationOptions.id, confirmedEventOptionId),
          ),
      () =>
        database
          .delete(schema.eventRegistrationOptions)
          .where(
            eq(schema.eventRegistrationOptions.id, checkedInEventOptionId),
          ),
      () =>
        database
          .delete(schema.eventInstances)
    ...
    if (failures.length > 0) {
      throw new AggregateError(failures, 'Profile event-card cleanup failed', {
        cause: failures[0],
      });
    }
  };
  ...
  const sourceEvent = await database.query.eventInstances.findFirst({
    where: { id: sourceEventId, tenantId: seeded.tenant.id },
    with: { registrationOptions: true },
  });
  const paidSourceEvent = await database.query.eventInstances.findFirst({
    where: { id: paidSourceEventId, tenantId: seeded.tenant.id },
    with: { registrationOptions: true },
  });
  ...
  const seedWriteFailures: unknown[] = [];
  try {
    await database.transaction(async (transaction) => {
      try {
        ...
        seedWriteFailures.push(error);
        throw error;
      }
    });
  } catch (error) {
    if (seedWriteFailures.length > 0 && seedWriteFailures[0] !== error) {
