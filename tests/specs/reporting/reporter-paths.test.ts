...
test('documentation reporter rejects raw markdown image syntax', async ({}, testInfo) => {
  ...
  expect(() =>
    ...
    'Documentation markdown attachment in Raw markdown image must not include raw Markdown image syntax, including reference-style images, raw HTML visual/media tags, or raw CSS image URLs.',
  );
});
...
test('documentation reporter rejects reference-style markdown image syntax', async ({}, testInfo) => {
  ...
  expect(() =>
    ...
    'Documentation markdown attachment in Reference markdown image must not include raw Markdown image syntax, including reference-style images, raw HTML visual/media tags, or raw CSS image URLs.',
  );
});
...
test('documentation reporter rejects raw HTML image tags', async ({}, testInfo) => {
  ...
  expect(() =>
    ...
    'Documentation markdown attachment in Raw HTML image must not include raw Markdown image syntax, including reference-style images, raw HTML visual/media tags, or raw CSS image URLs.',
  );
});
...
test('documentation reporter rejects raw HTML visual media tags', async ({}, testInfo) => {
  ...
  expect(() =>
    ...
    'Documentation markdown attachment in Raw HTML visual media must not include raw Markdown image syntax, including reference-style images, raw HTML visual/media tags, or raw CSS image URLs.',
  );
});

test('documentation reporter rejects raw CSS image URLs', async ({}, testInfo) => {
  const docsRoot = testInfo.outputPath('docs-out-raw-css-image');
  const imgsRoot = testInfo.outputPath('docs-img-raw-css-image');
  process.env.DOCS_OUT_DIR = docsRoot;
  process.env.DOCS_IMG_OUT_DIR = imgsRoot;
    ...
    attachments: [
      {
        ...
        contentType: 'text/markdown',
        body: Buffer.from(
          'This product guide must not embed <section style="background-image: url(../raw.png)">raw proof</section> outside the screenshot helper.',
        ),
      },
    ],
  } as any;
  ...
  expect(() =>
    reporter.onTestEnd({ title: 'Raw CSS image URL' } as any, result),
  ).toThrow(
    'Documentation markdown attachment in Raw CSS image URL must not include raw Markdown image syntax, including reference-style images, raw HTML visual/media tags, or raw CSS image URLs.',
  );
});
