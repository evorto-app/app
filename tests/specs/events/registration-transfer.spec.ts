...
test('transfers a free registration through a private transfer code', async ({
  browser,
  database,
  ...
  await openRegistrationTransferClaim(
    recipientPage.page,
...
test('offers a paid registration privately while rejecting a source self-claim', async ({
  ...
  if (!originalTenant?.stripeAccountId) {
  ...
  await claimRegistration.click();
  await expect(
    page.getByRole('heading', { name: 'Ticket could not be accepted' }),
  ).toBeVisible();
  await expect(
    page.getByText(
...
test('completes a paid transfer and preserves its failed refund for operator requeue', async ({
  ...
  expect(scenario.recipientRegistrationId).toBe(scenario.sourceRegistrationId);
  ...
  const fulfillmentEventsBefore = await database
    ...
    .where(
      and(
        ...
        eq(schema.eventRegistrationAddonFulfillmentEvents.tenantId, tenant.id),
      ),
    )
  ...
  const registrationsAfter = await database
    .select({
      appliedDiscountedPrice: schema.eventRegistrations.appliedDiscountedPrice,
  ...
  const refundTransactionIds = refundPlans.flatMap(({ refundTransactionId }) =>
...
test('refunds an inherited add-on from the recipient Checkout through its persisted payment account', async ({
  database,
  registerDatabaseCleanup,
  seeded,
  tenant,
  ...
  registerDatabaseCleanup(() => scenario.cleanup());
  expect(scenario.sourceStripeAccountId).toBe(scenario.stripeAccountId);
  const sourceAcquisitionBefore =
    await database.query.registrationAcquisitions.findFirst({
      where: {
        id: scenario.sourceAcquisitionId,
