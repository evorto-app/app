...
test.describe.skip('Checkout Tax Rate Integration', () => {
  ...
  test('checkout attaches tax rate identifier for compliance @finance @taxRates @checkout @track(playwright-specs-track-linking_20260126) @req(CHECKOUT-USES-TAX-RATE-ID-SPEC-03)', async ({
    page,
    events,
  }, testInfo) => {
    test.fixme(
  ...
  test('checkout continues when tax rate becomes unavailable @finance @taxRates @checkout @track(playwright-specs-track-linking_20260126) @req(CHECKOUT-USES-TAX-RATE-ID-SPEC-05)', async ({
    page,
    events,
  }) => {
  ...
  test('free registration skips tax rate processing @finance @taxRates @checkout @track(playwright-specs-track-linking_20260126) @req(CHECKOUT-USES-TAX-RATE-ID-SPEC-07)', async ({
    page,
    events,
    registrations,
    tenant,
  ...
  test('discounted price to zero treated as free @finance @taxRates @checkout @track(playwright-specs-track-linking_20260126) @req(CHECKOUT-USES-TAX-RATE-ID-SPEC-09)', async ({
    page,
    events,
  }, testInfo) => {
    test.fixme(
  ...
  test('payment request includes correct tax rate metadata @finance @taxRates @checkout @track(playwright-specs-track-linking_20260126) @req(CHECKOUT-USES-TAX-RATE-ID-SPEC-11)', async ({
    page,
    events,
  }) => {
  ...
  test('inactive tax rate warning logged during checkout @finance @taxRates @checkout @track(playwright-specs-track-linking_20260126) @req(CHECKOUT-USES-TAX-RATE-ID-SPEC-13)', async ({
    page,
    events,
  }, testInfo) => {
    test.fixme(
