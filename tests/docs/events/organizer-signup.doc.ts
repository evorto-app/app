...
Select **Sign up as organizer/helper**. A free first-come, first-served category confirms immediately when the choice is still available to you and a place remains. The event page then shows **Organizer/helper ticket confirmed**, your organizer/helper pass, and **Organize this event**.
...
`,
    ...
    await expect(
      activeRegistration.getByText('Organizer/helper ticket confirmed', {
        exact: true,
      }),
    ).toBeVisible();
    await expect(
...
The event page says **Organizer/helper application pending**. You cannot open the organizer overview, and no QR pass is available yet.
...
`,
      ...
      await expect(
        reviewer.page.getByText('Ticket confirmed', { exact: true }),
      ).toBeVisible({ timeout: 20_000 });
...
Open the applicant's event again after approval. This free application now shows **Organizer/helper ticket confirmed**, the organizer/helper pass, and **Organize this event**. A paid category remains pending until payment succeeds; the pass and organizer tools do not appear before then.
`,
      });
      ...
      await waitForRegistrationPage(page);
      await expect(
        page.getByText('Organizer/helper ticket confirmed', {
          exact: true,
        }),
      ).toBeVisible({ timeout: 20_000 });
      await expect(
