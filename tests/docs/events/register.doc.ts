...
test.describe('Sign up for events', () => {
  ...
  **Includes 1 guest plus you.** means the guest remains attached to the signed-in attendee's ticket and the two people use two confirmed places. Selected add-ons appear on the same ticket and organizers can review the answers.
  ...
  You can withdraw an application before approval. A pending payment can be cancelled only when Evorto confirms that it can safely release the held place. If payment setup needs review, keep the existing sign-up and contact an organizer; do not start another payment. You can also cancel a confirmed ticket before its displayed cancellation deadline and before the event starts. Cancelling a confirmed ticket releases your selected places, including guests, and starts any refund shown in the confirmation.`,
    });
  ...
  For a paid add-on, choose the quantity and select **Continue to payment**. Evorto holds those items while payment is pending. Leaving the payment page does not add them to the ticket: return to this event and continue the same payment instead of starting another purchase.`,
    ...
    await testInfo.attach('markdown', {
      body: `
  A paid add-on first holds the selected quantity and shows **Payment is pending**. It becomes available only after payment succeeds. While the payment page is active, use **Continue to payment** to return to the existing payment. If the link is unavailable or has expired, follow the message on the ticket and ask an organizer for review; do not start another purchase. Cancellation and transfer stay unavailable so the ticket cannot change at the same time.`,
    });
    await takeScreenshot(
      testInfo,
      page.locator('app-event-active-registration'),
