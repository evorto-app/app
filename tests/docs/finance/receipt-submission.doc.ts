...
Keep the dialog open while Evorto adds the file, submits the receipt, and checks the receipt lists. The file and entered values stay visible; saving and closing are unavailable while this work is in progress.

If a message says receipt submission has not started, correct the retained file or values before trying again. A named validation or access problem also keeps your entries available for correction. If the submission outcome could not be confirmed, your file and entries stay in the dialog until you close it. Close the dialog and select **Show latest receipts** on the event page. **Add receipt** remains unavailable until these reads succeed. Check the latest receipt lists before starting another submission; a failed or paused read keeps the action unavailable and the read-only control can be used again. Do not submit the same expense again just because its response was lost.

If the receipt was submitted but its lists could not be updated, it is already saved. Close the dialog and load the event page again to see it; do not submit it again.
`,
  });
  await receiptDialog
