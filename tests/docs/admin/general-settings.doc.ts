...
test('Manage organization settings @admin', async ({
  ...
  if (!documentedStripeAccountId) {
    throw new Error(
      'Expected generated settings docs tenant to have payments ready',
    );
  }
...
Changes apply to the organization currently shown in Evorto. They do not change another organization's settings. Your organization name and website address cannot be edited here; contact Evorto support when those details need changing.
`,
  });
  ...
  await test.step('Save organization settings', async () => {
    ...
    await expect(
      settings.getByText(tenantRecord.name, { exact: true }),
    ).toBeVisible();
    await expect(
      settings.getByText(tenantRecord.domain, { exact: true }),
    ).toBeVisible();
    await expect(
      settings.getByRole('textbox', { name: 'Organization name', exact: true }),
    ).toHaveCount(0);
    await expect(
      settings.getByRole('textbox', { name: 'Website address', exact: true }),
    ).toHaveCount(0);
    await expect(
  ...
  await test.step('Upload and save appearance', async () => {
    ...
    await testInfo.attach('markdown', {
      body: `
## Upload or link your logo and tab icon

Choose **Upload logo** or **Upload tab icon** and select a supported file. Wait for the upload confirmation, then choose **Save appearance settings** to publish the change. Uploading a file alone does not change the saved logo or tab icon. You can also paste the complete web address of an image on another website into **Logo web address** or **Tab icon web address**, then save.

Logos accept PNG, JPEG, WebP, or GIF files up to 5 MB. Tab icons also accept ICO files. If the upload is rejected, choose a supported, non-empty file within the size limit and try again.
`,
    });
    ...
    expect(
      await database.query.tenants.findFirst({
        columns: { faviconUrl: true, logoUrl: true },
        where: { id: tenant.id },
      }),
    ).toEqual({
      faviconUrl: tenantRecord.faviconUrl,
  ...
  await test.step('Review legal page ownership', async () => {
    ...
    await expect(
      settings.getByRole('button', { name: 'Save legal pages' }),
    ).toBeEnabled();
    await expect(
      settings.getByRole('textbox', { name: 'Privacy policy web address' }),
    ).toHaveCount(0);
    await expect(
      settings.getByRole('textbox', { name: 'Privacy policy text' }),
    ).toHaveCount(0);
    await takeScreenshot(
      testInfo,
      settings,
  ...
  await test.step('Save payment settings', async () => {
    ...
    await expect(settings.getByLabel('Stripe account ID')).toHaveCount(0);
    await expect(settings.getByPlaceholder('acct_...')).toHaveCount(0);
    await expect(
      settings.getByText(documentedStripeAccountId, { exact: true }),
    ).toHaveCount(0);
  ...
  await expect
    .poll(async () =>
      database.query.tenants.findFirst({
        columns: { domain: true, name: true, stripeAccountId: true },
        where: { id: tenant.id },
      }),
    )
    .toEqual({
  ...
  await test.step('Read saved values after reopening each settings page', async () => {
    ...
    await expect(
      organizationSettings.getByText(tenantRecord.name, { exact: true }),
    ).toBeVisible();
    await expect(
      organizationSettings.getByText(tenantRecord.domain, { exact: true }),
    ).toBeVisible();
    await expect(
      organizationSettings.getByPlaceholder('Example Section'),
    ).toHaveValue(documentedEmailSenderName);
    await expect(
      organizationSettings.getByPlaceholder('events@section.example.org'),
    ).toHaveValue(documentedEmailSenderEmail);
    await expect(
      organizationSettings.getByRole('combobox', { name: 'Time zone' }),
    ).toContainText('Berlin time');

    await page.getByRole('link', { name: 'Sign-up rules' }).click();
    const registrationSettings = page.locator('app-registration-settings');
    await expect(
      registrationSettings.getByRole('spinbutton', {
        name: 'Active sign-up limit',
      }),
    ...
    await page.getByRole('link', { name: 'Payments' }).click();
    const paymentSettings = page.locator('app-payment-provider-settings');
    await expect(paymentSettings.getByLabel('Stripe account ID')).toHaveCount(
      0,
    );
    await expect(paymentSettings.getByPlaceholder('acct_...')).toHaveCount(0);
    await expect(
      paymentSettings.getByText(documentedStripeAccountId, { exact: true }),
    ).toHaveCount(0);
    await expect(
      paymentSettings.getByText('Paid sign-ups are ready.', { exact: true }),
    ).toBeVisible();
    await expect(
      paymentSettings
        .locator('mat-slide-toggle')
    ...
    await expect(
      paymentSettings.getByPlaceholder('https://esncard.org/'),
    ).toHaveValue(documentedBuyEsnCardUrl);

    await page.getByRole('link', { name: 'Appearance' }).click();
    const appearanceSettings = page.locator('app-appearance-settings');
    await expect(
      appearanceSettings.getByRole('textbox', { name: 'Logo web address' }),
    ).toHaveValue(documentedLogoUrl);
    await expect(
      appearanceSettings.getByRole('textbox', { name: 'Tab icon web address' }),
    ).toHaveValue(documentedFaviconUrl);
    await expect(
      appearanceSettings.getByPlaceholder(
        'Organization name or public site title',
      ),
    ).toHaveValue(documentedSeoTitle);
    await expect(
      appearanceSettings.getByPlaceholder(
        'Short description for search results and previews',
      ),
    ).toHaveValue(documentedSeoDescription);

    await page.getByRole('link', { name: 'Legal pages' }).click();
    const legalSettings = page.locator('app-legal-settings');
    await expect(
      legalSettings.getByRole('link', { name: 'New member setup' }),
    ).toBeVisible();
    await expect(
      legalSettings.getByRole('button', { name: 'Save legal pages' }),
    ).toBeEnabled();
    for (const field of [
      {
        label: 'Imprint / legal notice web address',
        value: tenantRecord.legalNoticeUrl,
      },
      {
        label: 'Imprint / legal notice text published by Evorto',
        value: tenantRecord.legalNoticeText,
      },
      { label: 'Terms web address', value: tenantRecord.termsUrl },
      {
        label: 'Terms text published by Evorto',
        value: tenantRecord.termsText,
      },
    ]) {
      await expect(
        legalSettings.getByRole('textbox', { name: field.label, exact: true }),
      ).toHaveValue(field.value ?? '');
    }
    await expect(
      legalSettings.getByRole('textbox', {
        name: 'Privacy policy web address',
      }),
    ).toHaveCount(0);
    await expect(
      legalSettings.getByRole('textbox', { name: 'Privacy policy text' }),
    ).toHaveCount(0);
  });
...
Each page confirms when its settings have been saved and the current settings have been loaded. If **Save** is unavailable, correct the highlighted fields or wait for the current save to finish. If Evorto cannot confirm whether a save completed, check the saved settings before making another change. If the settings were saved but the current values could not be loaded, Evorto says so and keeps your entries. **Load saved settings** replaces those retained entries with the saved values. If you still cannot check the saved settings, contact Evorto support and include the settings page and exact message shown. Evorto does not replace your choice with another value.
...
`,
  ...
  await expect(page).toHaveURL(/\/admin\/onboarding$/u);
  ...
  await expect(
    onboardingSettings.getByText(
      /every member, including you, must\s+accept it before continuing/u,
    ),
  ).toBeVisible();
  await takeScreenshot(
    testInfo,
