...
One-domain-per-tenant remains the current relaunch scope in the application schema. The page now exposes the active primary domain and canonical root URL for operator review, allows tenant admins to maintain supported currency, locale, timezone, email reply-to settings, Stripe account id, registration limits, uploaded or externally hosted logo/favicon assets, legal links, and hosted legal text, and keeps an in-app deferred-settings summary for custom domain verification. Currency, locale, and timezone changes are only accepted before event or payment data exists for the tenant. When one of those accepted changes is saved, Evorto reloads the app so bootstrap-level formatting defaults use the new tenant settings.
`,
  });
});
