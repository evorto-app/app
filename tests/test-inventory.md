...
- `helpers/testing/generated-documentation-source.spec.ts` keeps tenant general-settings
  ...
  returned attach-helper factories, `call(...)`, `apply(...)`, inline
  `bind(...)(...)`, and `Reflect.apply(...)`, and rejects per-section source
  bodies shorter than 120 static characters so one long section cannot mask a
  thin generated-doc section in the same file. It also rejects screenshots
  ...
  returned attach-helper factories, `call(...)`, `apply(...)`, inline
  `bind(...)(...)`, and `Reflect.apply(...)`, and only real Playwright
  `testInfo.attach`-backed Markdown calls count as explanatory generated-doc
  sections, rejects blank
