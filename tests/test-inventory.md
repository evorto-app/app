...
- Events and registrations:
  - `docs/events/**`
  - `docs/events/register.doc.ts` asserts the successful free and paid
...
- Tenant/global admin:
  - `docs/admin/general-settings.doc.ts` covers the tenant general-settings
    ...
    fields, logo/favicon file-input accept lists, the legal-pages surface with
    legal URL/text fields,
...
- Profile and account:
  ...
  - `docs/users/create-account.doc.ts`
    ...
    and Create Account action instead of filtering the first matching page
    form, then reads back the created user, home tenant, tenant assignment,
    default role assignment, and post-create profile summary
...
- Runtime, reporting, screenshots, and seed health:
  ...
  - `specs/smoke/page-layout-helper.test.ts` exercises the shared viewport
    ...
    is the focused local rerun; it uses `helpers/testing/run-playwright.ts` to
    refresh `.env.dev`, set ignored docs/image output paths for the reporter,
    keep `NO_WEBSERVER=true`, and pass `--no-deps`.
  - `specs/smoke/public-general-viewports.spec.ts` reuses one seeded tenant to
    ...
    intentional horizontal scroll containers, retrying no stale visible
    `Loading...` or `Loading…` placeholders after expected route content renders,
    and no browser console
    ...
    guarded scan-flow entrypoint, and it checks the visible Events/Login labels
    plus the desktop-only Scanner navigation label alongside all hosted legal,
    ...
    headings, supporting copy, and fixed navigation text stay readable on
    painted Material surfaces instead of inheriting low-contrast fallback
    styling, while also failing on browser warning/error logs during that
    readability matrix. A matching
    ...
    rerun for an already-running Docker app; it uses
    `helpers/testing/run-playwright.ts` to refresh `.env.dev`, set ignored
    docs/image output paths, set `NO_WEBSERVER=true`, pass `--no-deps`, and
    keep the route matrix on one worker. `/create-account` is
  ...
  - `support/utils/page-layout.ts` provides the shared viewport layout guard for
    ...
    with edge and position labels, stale visible `Loading...` or `Loading…`
    placeholders after route content is expected to have settled, and overflowing visible
...
- `specs/templates/paid-option-requires-tax-rate.spec.ts` has active
  ...
  fixme declarations were removed; page-backed empty compatible-rate coverage
  opens the paid template tax-rate selector and asserts the visible
  no-compatible-inclusive-rates feedback. Template detail paid-option summaries
  now share the inclusive price label component with event
...
- `helpers/testing/generated-documentation-source.spec.ts` keeps tenant general-settings
  ...
  attachment, and the reporter runtime now enforces the same 120-character
  explanatory-body minimum before writing generated pages so a missed dynamic
  ...
  and derives the allowed generated-doc feature folders from the root
  `QUALITY.md` feature-area list,
  ...
  aliases such as `page.locator(shellSelector)` and constant-backed method
  ...
  inside screenshot arguments and arrays, rejects
  ...
  single-control locators, constant-backed role/test-id targets, and
  constant-backed `getByRole`/`getByTestId` method aliases, rejects direct
  single-control-looking `getByTestId`
  ...
  access, unwraps `as const` and `satisfies` grouped target objects before alias
  collection, rejects local
  ...
  helpers and names through copied groups, indexed lists, parameter destructuring
  ...
  `call(...)`, `apply(...)`, inline `bind(...)(...)`, optional-call syntax for
  both weak-body and markdown-count checks, and `Reflect.apply(...)`, and
  rejects per-section source bodies shorter than 120
  ...
  before the first Markdown section and more than two screenshots after one
  Markdown section so generated images stay close to the explanatory text that
  gives them product context, follows aliased Markdown attachment helpers and
  ...
  so a generated guide cannot silently omit or weaken the expected
  user-permissions callout,
  ...
  payloads, nested `.at(...)` raw Markdown payloads, negative nested `.at(...)`
  raw Markdown payloads, and attach helpers,
  ...
  properties and nested `.at(...)` screenshot-function properties, negative
  nested `.at(...)` screenshot-function properties, shorthand or alias-valued grouped
  ...
  objects, direct nested indexed raw image payload objects, nested `.at(...)`
  raw image payload objects, negative nested `.at(...)` raw image payload objects,
  ...
  reflected helper/raw image calls, reflected grouped screenshot-helper
  properties, returned raw screenshot and image attach helper functions,
...
- Profile/account:
  ...
  - Docker-backed system-Chrome profile event-card review now passes against the
    ...
    Generated profile screenshots now focus the combined
    navigation/profile-summary overview, filtered edit-profile dialog with the
    personal, notification, and reimbursement fields, concrete profile summary,
    ...
    broad profile-page, nav-only, or raw dialog captures for those states. The
    profile events screenshot highlights the registrations section plus
    confirmed, pending-checkout, waitlisted, and checked-in cards so the
    generated image shows the states asserted by the surrounding docs text.
...
- Scanning/check-in:
  - Docker-backed system-Chrome organizer aggregate review now passes for the
    ...
    scanner guest check-in, and event-edit role-picker surfaces instead of
    page, section, or status-text headings.
...
- Roles/user management:
  ...
  - Keep app action icons on the Font Awesome component path. Local source
    ...
    guard pins Docker Buildx setup, the retrying best-effort BuildKit image
    pre-pull before Buildx setup, the dependency-only warm build target, and
  ...
  - Local shared coverage pins admin-facing permission labels and descriptions,
    ...
    Product.md tenant-defined/default-role boundary, the filtered read-only
    admin user-list screenshot, the role-list create-action screenshot, and the
    saved role detail screenshot after role creation. The
...
- `tests/setup/mcp-browser.seed.ts` is a no-dependency Playwright-test MCP planner seed for the dedicated `mcp-browser-planner` project. It uses the plain Playwright test API to open `/legal/terms`, so MCP Browser planning can initialize against a public General page without running the database/auth setup projects. `bun run test:e2e:mcp-browser-planner` is the focused local rerun for an already-running app; it refreshes `.env.dev`, sets `NO_WEBSERVER=true`, uses `--no-deps`, and keeps the public planner seed on one worker. The current Browser planner setup path has verified that project/seed pair, resized the seeded Terms page to 320x740, and captured a mobile screenshot with readable legal-page content plus fitting Events/Login bottom navigation. A later current-head Browser refresh on pushed head `574e9273` used alternate local ports to avoid an existing 4200 stack, then checked `/legal/terms` and `/events` at 320x740 plus `/404` and `/legal/terms` at 390x844; the screenshots showed readable legal/error text, meaningful seeded event-card icons and times, and no visible bottom-navigation clipping or overlap.
  ...
  running Docker app.
  A newer Browser refresh on pushed head `aef3ccdc` rechecked the same anonymous
  General route set at 320x740 and 390x844, derived the current public event
  detail link from `/events`, and reported no horizontal overflow, top/side
  clipped visible controls, persistent loading placeholders, or rendered
  application-error text. The inspected evidence images
  `/tmp/evorto-public-general-aef3ccdc-narrow-mobile-events.jpg` and
...
- Local workflow entrypoints stay visible in `package.json` for app build/dev,
  ...
  bootstrap/start/resume/webServer/stop, database commands, Neon Local branch
  cleanup, dependency updates, Stripe/Sentry ops, theme generation, and
  receipt-image cleanup.
...
- Runtime-preflight coverage also guards the non-secret runtime target summary
  ...
  warning both point at the guarded
  `bun run env:copy-main -- --if-missing` command while keeping generated
  `.env.dev` worktree-local and the main checkout `.npmrc` out of the worktree.
  `bun run env:bootstrap` is now the shared local bootstrap prelude that asks
  `env:copy-main --if-missing` to copy `.env` only when missing and then
  refreshes `.env.dev`; `bun run dev:bootstrap`, `bun run docker:bootstrap`,
  local Playwright package scripts, and `bun run dev:start` reuse that prelude
  before their normal preflight or app start. This keeps the missing-file
  decision inside the tested Bun helper instead of a package-script shell
  conditional, and keeps copy-if-missing plus runtime-env generation in one
  visible package script. The helper also no-ops before source-checkout lookup
  when `--if-missing` finds an existing worktree `.env`.
...
- `bun run dev:status` is the combined non-mutating local runtime status path.
  ...
  covers successful public-route probes, closed-port skips, other-checkout port
  ownership skips, HTTP-error failures, and missing `BASE_URL` skips.
...
- E2E CI Docker startup, teardown, and final Neon pruning live in
  ...
  prune steps in E2E run `27068078693`. Live repo-local Neon cleanup dry-runs
  returned to `total=1`,
  `protected=1`, `active_test=0`, `stale_deleted=0`, and `ttl=2h` after the
  completed current-head finalizers, so only protected `main` remained visible
  outside stale cleanup eligibility.
...
- A prior full-matrix in-app Browser public General refresh was captured on PR
  ...
  event dates into the future so Browser could inspect public event cards
  without test-cookie injection. The refined Browser sweep checked all 10
  anonymous General routes at 320x740, 390x844, and 1440x900: `/`, `/events`,
  seeded event detail `/events/09673e60b8e35d7cdc25`, `/legal/imprint`,
  `/legal/privacy`, `/legal/terms`, `/403`, `/500`, `/404`, and
  `/missing-general-page`. All 30 route/viewport checks found expected current
  content, no persistent loading or application-error text, no true horizontal
  overflow, no clipped fixed controls, correct Events/Login mobile navigation
