...
baseline after staging passes its main release smoke checks:
...
and remove its `continue-on-error`. A failed post-traffic performance check
must remain visibly failed. Preserve its evidence and ship a corrected forward
revision through the normal release gate; do not redeploy an older image after
a schema release.
...
8. If the regression begins at a deployment, compare the prior manifest and
   follow the documented forward-recovery process in the infrastructure README.
