...
import {
  ...
  eventRegistrationPurchaseOwnerUniqueConstraintName,
...
describe('registration question answer integrity', () => {
  ...
  it('binds each answer to one question and registration owner tuple', () => {
    ...
    expectUniqueConstraint({
      columns: ['id', 'eventId', 'registrationOptionId', 'tenantId'],
      name: eventRegistrationPurchaseOwnerUniqueConstraintName,
  ...
  it('requires the complete answer owner scope at insert time', () => {
    for (const column of [
      ...
      eventRegistrationQuestionAnswers.registrationOptionId,
      eventRegistrationQuestionAnswers.tenantId,
    ]) {
      expect(column.notNull).toBe(true);
      expect(column.hasDefault).toBe(false);
    }
