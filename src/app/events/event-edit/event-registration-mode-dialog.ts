...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      @if (data.to === 'advanced') {
        <p>
          Advanced setup keeps both current choices and lets you add sign-up
          choices and choose which choices can use each add-on. You can return
          to simple setup only while the event has exactly one organizer choice
          and one attendee choice.
        </p>
      } @else {
        <p>
