...
describe('admin load recovery', () => {
  it.each([
    {
      ...
      failurePath: 'src/app/admin/role-details/role-details.component.ts',
  ...
  ])('pairs the visible $path failure with a real retry action', (scenario) => {
    const { failure, loading, path, query } = scenario;
    const source = readFileSync(nodePath.join(process.cwd(), path), 'utf8');

    expect(source).toContain(loading);
    if ('failurePath' in scenario) {
      expect(source).toContain(`{{ errorMessage(${query}.error()) }}`);
      const messageSource = readFileSync(
        nodePath.join(process.cwd(), scenario.failurePath),
        'utf8',
      );
      expect(messageSource).toContain(failure);
    } else {
