                  ...
                  <td mat-cell *matCellDef="let rate">
                    @if (rate.percentage === '0') {
                      <mat-chip
                        ...
                        >Tax-free</mat-chip
                      >
                    } @else if (rate.percentage === null) {
                      <span class="body-medium">Percentage unavailable</span>
                    } @else {
...
export class TaxRatesSettingsComponent {
  ...
  protected readonly importedRates = computed(() => {
    const rates = this.importedQuery.isSuccess()
      ...
      : [];
    return rates.filter(
      (rate) => rate.inclusive && rate.active && rate.percentage !== null,
    );
  });
