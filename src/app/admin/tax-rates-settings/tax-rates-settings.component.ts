...
@Component({
  ...
  imports: [MatButtonModule, MatCardModule, MatChipsModule, MatIconModule, MatTableModule],
        ...
        <div class="bg-error-container text-on-error-container rounded-2xl p-4">
          <div class="flex items-center gap-2">
            ...
            <span class="body-medium"
              >Failed to load tax rates: {{ importedQuery.error()?.message }}</span
            >
          </div>
        </div>
      }
            ...
            <div class="bg-surface rounded-2xl overflow-hidden">
              <table mat-table [dataSource]="importedRates()" class="w-full">
                ...
                <ng-container matColumnDef="percentage">
                  ...
                  <td mat-cell *matCellDef="let rate">
                    @if (rate.percentage === '0') {
                      <mat-chip class="bg-tertiary-container text-on-tertiary-container"
                        >Tax Free</mat-chip
                      >
                    } @else {
                ...
                <ng-container matColumnDef="region">
                  ...
                  <td mat-cell *matCellDef="let rate">
                    @if (rate.country || rate.state) {
                      <span class="body-medium"
                        >{{ rate.country }}{{ rate.state ? ', ' + rate.state : '' }}</span
                      >
                    } @else {
                ...
                <ng-container matColumnDef="status">
                  ...
                  <td mat-cell *matCellDef="let rate">
                    <mat-chip class="bg-primary-container text-on-primary-container"
                      >Compatible</mat-chip
                    >
                  </td>
                </ng-container>
            ...
            <div class="bg-surface rounded-2xl overflow-hidden opacity-60">
              <table mat-table [dataSource]="incompatibleRates()" class="w-full">
                ...
                <ng-container matColumnDef="percentage">
                  ...
                  <td mat-cell *matCellDef="let rate">
                    @if (rate.percentage === '0') {
                      <mat-chip class="bg-surface-variant text-on-surface-variant"
                        >Tax Free</mat-chip
                      >
                    } @else {
                      <span class="body-medium">{{ rate.percentage }}%</span>
                ...
                <ng-container matColumnDef="status">
                  ...
                  <td mat-cell *matCellDef="let rate">
                    @if (!rate.inclusive) {
                      <mat-chip class="bg-error-container text-on-error-container"
                        >Exclusive</mat-chip
                      >
                    } @else if (!rate.active) {
                      <mat-chip class="bg-error-container text-on-error-container"
