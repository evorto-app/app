...
describe('profile payout serialization', () => {
  it('normalizes payout details while preserving notification email casing', () => {
...
const withPayoutDialog = async (
  ...
  try {
    ...
    await check({ close, field, fill, fixture: createdFixture, root });
  } catch (error) {
    failures.push(error);
  }
...
describe('profile payout form validation', () => {
  it('retains invalid entries until corrected and closes with encoder-compatible payout values', async () => {
    await withPayoutDialog(async ({ close, field, fill, fixture, root }) => {
      const iban = fill('IBAN (for reimbursements)', 'DE88370400440532013000');
      ...
      expect(root.textContent).toContain('Enter a valid PayPal email address.');
      fill('IBAN (for reimbursements)', ' nl91 abna 0417 1643 00 ');
      expect(save.disabled).toBe(true);
      fill('PayPal email (for reimbursements)', ' PayPal@Example.COM ');
      expect(save.disabled).toBe(false);
      expect(field('Notification email').value).toBe('Events@Example.COM');
      await fixture.componentInstance.onSubmit(new Event('submit'));
  ...
  it('keeps blank payout fields optional and preserves current required fields', async () => {
    await withPayoutDialog(async ({ close, fill, fixture }) => {
      ...
      fill('Notification email', 'invalid');
      await fixture.componentInstance.onSubmit(new Event('submit'));
      expect(close).not.toHaveBeenCalled();
      fill('Notification email', 'Events@Example.COM');
      await fixture.componentInstance.onSubmit(new Event('submit'));
      expect(close).toHaveBeenCalledExactlyOnceWith({
        communicationEmail: 'Events@Example.COM',
        firstName: 'Alice',
