...
type ProfileEventAction = Pick<
  ...
  | 'status'
>;
type ProfileEventPayment = Pick<
  UsersEventSummaryRecord,
  'checkoutUrl' | 'paymentState' | 'status'
>;
...
export const profileEventNextStepLabel = (
  event: ProfileEventPayment,
): null | string => {
...
export const profileEventContinuePaymentUrl = (
  event: ProfileEventPayment,
): null | string => {
  if (event.status === 'CANCELLED' || event.paymentState !== 'pending') {
    return null;
  }

  return normalizeStripeCheckoutUrl(event.checkoutUrl);
};

export const profileEventActionNote = (event: ProfileEventAction): string => {
  if (event.status === 'CANCELLED') {
