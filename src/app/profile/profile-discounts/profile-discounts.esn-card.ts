...
const esnCardFallbackMessages = {
  ...
  save: "We couldn't confirm whether this ESNcard was saved. Select Try again to load your current cards before making another change.",
} as const satisfies Record<EsnCardMutationAction, string>;

const esnCardRejectedMessages = {
  refresh:
...
export const isEsnCardChangedError = (error: unknown): boolean =>
  taggedErrorName(error) === 'DiscountCardChangedError';

export const isEsnCardUnconfirmedError = (error: unknown): boolean => {
  switch (taggedErrorName(error)) {
...
export const esnCardMutationErrorMessage = (
  ...
  switch (tag) {
    ...
    case 'RpcBadRequestError': {
      return esnCardRejectedMessages[action];
    }
    case 'RpcForbiddenError': {
