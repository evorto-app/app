...
describe('EmailOutboxComponent overview', () => {
  ...
  it('does not describe sent history with a missing attempt time as untried', async () => {
    loadOverview.mockResolvedValue({
      items: [
        {
          id: 'email-sent',
    ...
    await vi.waitFor(async () => {
      await fixture.whenStable();
      const text = normalizeText(fixture);
      expect(text).toContain('Evorto sent this message.');
      expect(text).toContain('Some delivery details are missing.');
      expect(text).toContain('Not recorded');
      expect(text).not.toContain('Not tried yet');
    });
  });
  ...
  it('renders each Brisbane row in Brisbane when the host tenant is Berlin', async () => {
    ...
    await vi.waitFor(async () => {
      ...
      expect(text).toContain(
        'The counts cover all emails. This list shows up to 100, placing emails that need attention first. Within each group, the most recently updated appear first.',
      );
    });
  });
