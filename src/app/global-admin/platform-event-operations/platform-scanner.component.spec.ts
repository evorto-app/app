...
const renderedElement = (element: unknown): HTMLElement => {
  if (!(element instanceof HTMLElement)) {
    throw new TypeError('Expected a rendered scanner element');
  }
  return element;
};
...
const findAlertButton = (
  ...
  [
    ...renderedElement(fixture.nativeElement).querySelectorAll<HTMLElement>(
      '[role="alert"]',
    ),
  ]
...
describe('PlatformScannerComponent', () => {
  ...
  > => {
    ...
    await vi.waitFor(() => {
      ...
      expect(renderedElement(fixture.nativeElement).textContent).toContain(
        'Weekend trip',
      );
    });
    const reason = renderedElement(
      fixture.nativeElement,
  ...
  it('retries a failed registration lookup', async () => {
    ...
    expect(renderedElement(fixture.nativeElement).textContent).not.toContain(
      'Provider secret',
    );
  ...
  it('clears action state when the inspected registration changes', async () => {
    const fixture = await render();
    const guestCount = renderedElement(
      fixture.nativeElement,
  ...
  it('describes attendee cancellation updates as an attempt', async () => {
    const fixture = await render();
    const text = renderedElement(fixture.nativeElement)
