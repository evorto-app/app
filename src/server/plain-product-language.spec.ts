...
const rpcContractsRoot = path.join(process.cwd(), 'src/shared/rpc-contracts');
...
const userVisibleTechnicalLanguage =
  /\b(?:apis?|authentication|authorized|conditional|configur(?:ation|e[ds]?|ing)|databases?|domains?|eligib(?:le|ility)|endpoints?|fallbacks?|metadata|payloads?|providers?|rpcs?|schemas?|tenants?|urls?|webhooks?)\b|access denial|internal id|operator reason|platform administrator|request failure|source of truth|visibility setting/giu;

const publicErrorTechnicalLanguage =
  /\b(?:apis?|authentication|authorized|browser|conditional|configur(?:ation|e[ds]?|ing)|databases?|domains?|eligib(?:le|ility)|endpoints?|fallbacks?|metadata|payloads?|providers?|refresh(?:ed|es|ing)?|rpcs?|schemas?|tenants?|urls?|webhooks?)\b|access denial|forbidden|internal id|internal server error|operator reason|platform administrator|request failure|source of truth|stable identifier|visibility setting/giu;
...
describe('plain server-supplied product language', () => {
  it('does not ask users to repair stale state through browser mechanics', async () => {
    const renderedEmails = await Promise.all(
      emailExamples().map((email) => render(email, { plainText: true })),
    );
    const authoredServerCopy = productionSources(serverRoot).flatMap(
      (sourcePath) => [
        ...authoredPublicRpcErrorCopy(sourcePath),
        ...['failureMessage', 'publicMessage', 'subject'].flatMap(
          (propertyName) => authoredPropertyCopy(sourcePath, propertyName),
        ),
      ],
    );
    const visibleCopy = [
      ...renderedEmails.map((text, index) => ({
        source: `email ${index + 1}`,
        text,
      })),
      {
        source: 'unknown organization page',
        text: publicDocumentText(unknownTenantDocument),
      },
      ...authoredServerCopy.map(({ line, text }) => ({
        source: `server source:${line}`,
        text,
      })),
    ];
    const violations = visibleCopy.flatMap(({ source, text }) =>
      [...text.matchAll(browserMechanicsAndVaguePromises)].map(
  ...
  it('keeps authored RPC contract messages in product language', () => {
    const violations = productionSources(rpcContractsRoot).flatMap(
      (sourcePath) =>
        authoredPropertyCopy(sourcePath, 'message').flatMap(({ line, text }) =>
          [...text.matchAll(publicErrorTechnicalLanguage)].map(
            (match) =>
              `${path.relative(process.cwd(), sourcePath)}:${line}: ${match[0]}`,
          ),
