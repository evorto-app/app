...
class UnexpectedPaymentSetupHttpClient extends Stripe.HttpClient {
  override getClientName(): string {
    return 'evorto-invalid-payment-setup-route-test';
  }

  override makeRequest(): Promise<Stripe.HttpClientResponse> {
    return Promise.reject(
      new Error(
        'Unexpected Stripe access before payment setup input validation',
      ),
    );
  }
}
...
const routeLayer = (role: 'web' | 'worker') => {
  ...
  const requestLayer = Layer.mergeAll(
    ...
    Layer.succeed(
      StripeClient,
      new Stripe('sk_test_payment_setup_route', {
        httpClient: new UnexpectedPaymentSetupHttpClient(),
        maxNetworkRetries: 0,
      }),
    ),
  );
