...
const applyPolicy = (
  request: Request,
  options: RpcIngressPolicyOptions = defaultPolicyOptions,
) => {
...
describe('runRpcIngressPolicy', () => {
  ...
  it('does not use an ambient SSR origin', () => {
    vi.stubEnv('SSR_RPC_ORIGIN', 'http://127.0.0.1:4200');
    try {
      const { handler, result } = applyPolicy(
        makeRequest({
          cookie: 'appSession=session',
  ...
  it('allows the proven no-Origin SSR caller on its configured loopback origin', () => {
    ...
    expect(handler).toHaveBeenCalledWith({
      trustedTenantDomain: 'tenant.example.com',
    });
  });
  ...
  it('does not trust tenant routing headers from an ordinary browser request', () => {
    const { handler, result } = applyPolicy(
      makeRequest({
        cookie: 'appSession=session',
        headers: {
