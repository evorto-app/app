...
export async function computeIconSourceColor(
  ...
  try {
    ...
    const promise = (async () => {
    // Temporarily disabled: canvas not available in production
    // TODO: Re-enable when canvas dependencies are needed
    console.warn(`Canvas-based icon color computation disabled for: ${iconCommonName}`);
    return undefined;
    
    // Original canvas-based implementation (commented out):
