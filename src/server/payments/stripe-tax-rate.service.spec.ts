...
describe('Stripe tax-rate service', () => {
  ...
  it.effect('rejects a rate without a percentage during import', () =>
    Effect.gen(function* () {
      const stripeTaxRates: StripeTaxRateApi = {
        list: unusedList,
        retrieve: (id) => Promise.resolve(stripeRate(id, { percentage: null })),
      };
      ...
      expect(error).toMatchObject({
        _tag: 'RpcBadRequestError',
        message:
          'A selected tax rate has no percentage. No tax rates were added. Choose a percentage-based rate that is included in the shown price.',
        reason: 'taxRatePercentageRequired',
      });
    }),
  );

  it.effect('maps a missing or foreign-account rate ID to a typed error', () =>
    Effect.gen(function* () {
