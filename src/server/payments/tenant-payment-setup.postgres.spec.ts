...
describe('tenant payment setup PostgreSQL serialization', () => {
  ...
  afterAll(async () => {
    ...
    if (failures.length > 0)
      throw new AggregateError(
        failures,
        'Payment setup fixture cleanup failed',
      );
  });
  ...
  it('serializes two concurrent first attachments so one wins and audits once', async () => {
    ...
    if (failures.length > 0)
      throw new AggregateError(
        failures,
        'Concurrent payment setup proof failed',
      );
  }, 30_000);
  ...
  it('allows one payment account to serve more than one organization', async () => {
    const suffix = randomUUID().replaceAll('-', '').slice(0, 8);
    const sharedAccountId = `acct_shared_${suffix}`;
    const organizations = [
      {
        domain: `${suffix}-first.shared-payment.example`,
        id: `shared-a-${suffix}`,
        name: `Shared payment first ${suffix}`,
      },
      {
        domain: `${suffix}-second.shared-payment.example`,
        id: `shared-b-${suffix}`,
        name: `Shared payment second ${suffix}`,
      },
    ] as const;
    fixtures.push(...organizations.map(({ id: tenantId }) => ({ tenantId })));
    await database.insert(tenants).values([...organizations]);
        ...
        organizations.map((organization) =>
          attachTenantPaymentAccount(
            makeInput({
              ...
              reason: `Initial paid sign-up approval for ${organization.name}`,
            }),
          ),
        ),
        { concurrency: 'unbounded' },
    ...
    expect(outcomes).toEqual([{ attached: true }, { attached: true }]);
    ...
    for (const organization of organizations) {
      ...
      ).toMatchObject({ stripeAccountId: sharedAccountId });
    }
  });
