...
const lockFulfillmentRows = Effect.fn('lockFulfillmentRows')(function* (
  ...
  if (registration.status !== 'CONFIRMED') {
    return yield* conflict(
      'Add-ons can only be changed for a confirmed ticket.',
    );
  }
          ...
          Effect.gen(function* () {
            ...
            const currentAcquisition =
              yield* lockCurrentRegistrationAcquisition(tx, {
                ...
                Effect.mapError(() =>
                  conflict(
                    "Evorto could not verify this ticket's add-on details. No changes were made. Ask an administrator to review the ticket.",
                  ),
                ),
              );
            if (
