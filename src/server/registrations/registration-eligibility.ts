...
export const lockCurrentRegistrationEligibility = Effect.fn(
  ...
  input: {
    ...
    readonly tenantLockMode: 'key share' | 'update';
    readonly userId: string;
  },
) {
  // Existing registrations must be locked by the caller before membership.
  // Take the final tenant lock strength before option/event locks: payment
  // reservations also lock the tenant first, and inserts need FK key-share.
  // Choosing UPDATE up front avoids concurrent key-share-to-update upgrades.
  yield* tx
    .select({ id: tenants.id })
    .from(tenants)
    .where(eq(tenants.id, input.tenantId))
    .for(input.tenantLockMode);

  const questions = yield* lockEventRegistrationQuestionSet(tx, {
    ...input,
    eventLockMode: 'update',
  });
  if (!questions)
    return {
