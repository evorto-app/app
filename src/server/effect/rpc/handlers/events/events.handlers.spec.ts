...
const eventAddonTypesSql =
  'select "d0"."id" as "id" from "event_addons" as "d0" where ((exists (select * from "event_instances" as "f0" where (("d0"."eventId" = "f0"."id") and ("f0"."tenantId" = $1)) limit 1)) and ("d0"."eventId" = $2)) limit $3';
...
const createEventDiscountDatabase = ({
  ...
  status?: typeof eventInstances.$inferSelect.status;
  taxScenario?: TaxScenario;
} = {}) => {
  const findAddons = vi.fn<(query: EventSqlQuery) => void>();
  ...
  const taxRate = {
    displayName: taxScenario === 'nullDisplayName' ? null : 'VAT',
    percentage: taxScenario === 'nullPercentage' ? null : taxPercentage,
    stripeTaxRateId: 'txr_option',
  } satisfies Pick<
    typeof tenantStripeTaxRates.$inferSelect,
    'displayName' | 'percentage' | 'stripeTaxRateId'
  >;
  const optionIds = mappingCount > 20 ? ['option-1', 'option-2'] : ['option-1'];
  const optionFilterSql = `in (${optionIds.map((_, index) => `$${index + 2}`).join(', ')})`;
  const mappedAddonsSql = eventAddonsSql.replace(
    'in ($2)',
    () => optionFilterSql,
  );
  const questionsSql = eventQuestionsSql.replace(
    'in ($2)',
    () => optionFilterSql,
  );
  const discountsSql = optionDiscountsSql.replace(
    'in ($2)',
    () => optionFilterSql,
  );
  const option = {
    ...
    executeValues: (statement, parameters) =>
      Effect.sync(() => {
        ...
        if (statement === eventAddonTypesSql) {
          ...
          findAddons({ parameters, statement });
          return Array.from({ length: addonCount }, (_, index) => [
            `addon-${index}`,
          ]);
        }
        if (statement === mappedAddonsSql) {
          expect(parameters).toEqual([event.id, ...optionIds]);
          selectDetails({ parameters, statement });
          return Array.from({ length: mappingCount }, (_, index) => [
            true,
            false,
            false,
            true,
            ...
            taxScenario === 'optionalAddonWithoutTax' ? 500 : 0,
            `option-${1 + Math.floor(index / 20)}`,
            null,
            'Included item',
            100,
          ]);
        }
        if (statement === questionsSql) {
          expect(parameters).toEqual([event.id, ...optionIds]);
          selectDetails({ parameters, statement });
        ...
        if (statement === discountsSql) {
          expect(parameters).toEqual(['esnCard', ...optionIds]);
          selectDetails({ parameters, statement });
          return optionIds.map((id) => [1000, 'esnCard', id]);
        }
        if (statement === optionTaxRatesSql) {
          expect(parameters).toEqual([tenant.id, 'acct_tenant', 'txr_option']);
          readTaxRates({ parameters, statement });
          return taxScenario === 'missingRow'
            ? []
            : [
                [
                  taxRate.displayName,
...
describe('event discount tenant isolation', () => {
  ...
  ] satisfies readonly Parameters<typeof createEventDiscountDatabase>[0][]) {
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(findAddons).toHaveBeenCalledExactlyOnceWith({
            parameters: [tenant.id, 'event-1', 21],
            statement: eventAddonTypesSql,
          });
          if (addonCount > 20) {
            expect(Result.isFailure(result)).toBe(true);
            if (Result.isFailure(result))
          ...
          if (!hiddenOptions && optionTaxRateId) {
            expect(readTaxRates).toHaveBeenCalledExactlyOnceWith({
              parameters: [tenant.id, 'acct_tenant', 'txr_option'],
              statement: optionTaxRatesSql,
            });
          } else {
          ...
          if (mappingCount === 40) {
            expect(event.registrationOptions).toHaveLength(2);
            for (const addOn of event.addOns) {
              expect(addOn.registrationOptions).toEqual([
                {
                  includedQuantity: 1,
