...
const createManualApprovalDatabase = ({
      ...
      select: () => ({
        from: (table: unknown) => ({
          where: () => {
            if (table === eventRegistrations) {
              return {
                for: () => Effect.succeed([{ status: 'PENDING' }]),
              };
            }

            if (table === tenants) {
              return {
                for: () => Effect.succeed([{ stripeAccountId: 'acct_123' }]),
              };
            }
  ...
  const database = {
    ...
    transaction: (
      ...
      transactionCount += 1;
      return callback(createTransaction(transactionCount > 1));
    },
  };
