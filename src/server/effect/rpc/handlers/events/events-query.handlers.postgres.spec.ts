...
import { PlatformEventsUpdateAnnouncementDiscovery } from '../../../../../shared/rpc-contracts/app-rpcs/platform-events.rpcs';
import { PlatformRolesDelete } from '../../../../../shared/rpc-contracts/app-rpcs/platform-tenant-admin.rpcs';
...
const runEventList = (
  ...
  Effect.runPromise(
    eventQueryHandlers['events.eventList'](
      ...
      createRpcOptions(EventsEventList.middleware(RpcRequestContextMiddleware)),
    ).pipe(
...
const runFindOne = (context: RpcRequestContextShape, id: string) =>
  Effect.runPromise(
    eventQueryHandlers['events.findOne'](
      { id },
      createRpcOptions(EventsFindOne.middleware(RpcRequestContextMiddleware)),
    ).pipe(
      Effect.provideService(RpcRequestContext, context),
      Effect.provide(handlerLayer),
    ),
...
const runPlatformAnnouncementDiscoveryUpdate = (
  ...
  Effect.runPromise(
    platformEventHandlers['platform.events.updateAnnouncementDiscovery'](
      ...
      createRpcOptions(
        PlatformEventsUpdateAnnouncementDiscovery.middleware(
          RpcRequestContextMiddleware,
        ),
      ),
    ).pipe(
...
const runRoleDeletion = () =>
  Effect.runPromise(
    platformTenantAdminHandlers['platform.roles.delete'](
      ...
      createRpcOptions(
        PlatformRolesDelete.middleware(RpcRequestContextMiddleware),
      ),
    ).pipe(
      Effect.match({
        onFailure: (error) => ({ error, status: 'failure' as const }),
...
const waitForBlockedRoleGraphLocks = async (
  pool: Pool,
  minimumCount: number,
  ensureWorkersUnsettled: () => void,
) => {
  const deadline = Date.now() + 10_000;
  while (Date.now() < deadline) {
    ensureWorkersUnsettled();
...
describe('optionless announcement discovery', () => {
  ...
  let pool: Pool;
  let cleanupDatabase: TestDatabase | undefined;
  ...
  afterAll(async () => {
    const databaseToClean = cleanupDatabase;
    const poolToEnd = cleanupPool;
        ...
        ? [
            () =>
              databaseToClean
                ...
                .where(
                  inArray(platformAuditEntries.targetTenantId, [
                    tenantId,
                    otherTenantId,
                  ]),
                ),
            ...
            () =>
              databaseToClean
                ...
                .where(
                  inArray(eventTemplates.id, [templateId, otherTemplateId]),
                ),
            () =>
              databaseToClean
                .delete(eventTemplateCategories)
                .where(
                  inArray(eventTemplateCategories.id, [
                    categoryId,
                    otherCategoryId,
                  ]),
                ),
            () =>
              databaseToClean
                ...
                .where(
                  inArray(rolesToTenantUsers.tenantId, [
                    tenantId,
                    otherTenantId,
                  ]),
                ),
            () =>
              databaseToClean
                ...
                .where(
                  inArray(usersToTenants.tenantId, [tenantId, otherTenantId]),
                ),
            () =>
              databaseToClean
                .delete(roles)
            ...
            () =>
              databaseToClean.delete(users).where(inArray(users.id, userIds)),
            () =>
              databaseToClean
                .delete(tenants)
    ...
    if (failures.length > 0) {
      throw new AggregateError(
        failures,
        'Announcement discovery fixture cleanup failed',
        { cause: failures[0] },
      );
    }
  });
  ...
  it('serializes platform announcement updates with role deletion', async () => {
    ...
    const workers: { name: string; settled: boolean }[] = [];
    const recordFailure = (error: unknown) => {
      if (!failures.includes(error)) failures.push(error);
    };
    const observeWorker = <Result>(
      name: string,
      operation: Promise<Result>,
    ) => {
      const worker = { name, settled: false };
      workers.push(worker);
      operations.push(operation);
      // Observe immediately, before waiting for either lock barrier.
      void operation.then(
        () => {
          worker.settled = true;
        },
    ...
    const ensureWorkersUnsettled = () => {
      if (failures.length > 0) throw failures[0];
      const completed = workers.find((worker) => worker.settled);
      if (completed) {
        throw new Error(
          `${completed.name} completed before its role-graph lock was observed`,
        );
      }
    };
    try {
      await lockTenantRoleGraph(lockClient, tenantId);
      const updatePromise = observeWorker(
        'Announcement update',
        runPlatformAnnouncementDiscoveryUpdate(raceAnnouncementId, [
          raceRoleId,
        ]),
      );
      await waitForBlockedRoleGraphLocks(pool, 1, ensureWorkersUnsettled);
      const deletePromise = observeWorker('Role deletion', runRoleDeletion());
      await waitForBlockedRoleGraphLocks(pool, 2, ensureWorkersUnsettled);
      ...
      try {
        // Destroy a client after a failed rollback so it cannot retain the lock.
      ...
      for (const settlement of settlements) {
        if (settlement.status === 'rejected') recordFailure(settlement.reason);
      }
    }
    if (failures.length > 0) {
      throw new AggregateError(
        failures,
        'Announcement discovery role-lock scenario failed',
        { cause: failures[0] },
      );
    }
  });
