...
export const financeMediaHandlers = {
  ...
  'finance.receiptMedia.finalizeUpload': (input, _options) =>
    Effect.gen(function* () {
      ...
      // Record the destination before writing it so interruption or an unknown
      // database outcome always leaves the promoted object owned by cleanup.
      const boundDestinations = yield* Database.use((database) =>
        database
          .update(financeReceiptUploads)
          .set({ storageKey: inspected.storageKey })
          .where(
            and(
              eq(financeReceiptUploads.id, upload.id),
              eq(financeReceiptUploads.tenantId, tenant.id),
        ...
        mapStorageMutationError(
          'receiptMedia.upload.bindDestination',
          'Failed to record the receipt upload destination',
        ),
      );
      if (boundDestinations.length !== 1) {
