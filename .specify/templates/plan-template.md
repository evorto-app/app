...
- CLI per library: OPTIONAL (add only if it materially helps debugging or review)
- Library docs: llms.txt format planned?

**Testing (NON-NEGOTIABLE)**:
- RED-GREEN-Refactor cycle enforced? (test MUST fail first)
- Git commits show tests before implementation?
- Order: E2E→Contract→Integration→Unit strictly followed?
- E2E validates constraints (auth, roles, perf budgets, a11y where applicable)?
- Documentation tests (`*.doc.ts`) included where appropriate to generate living docs?
- Real dependencies used? (actual DBs, not mocks)
- FORBIDDEN: Implementation before test, skipping RED phase

**Legacy Migration (data‑only)**:
- Migration Impact section present in plan.md?
  - Data mapping (old DB → new DB): tables/fields and mapping rules
  - Defaults/backfills for new fields; idempotency strategy; verification checks
  - Failure handling and (optional) rollback approach for the data import
  - Seed data updates required so the feature is testable without migration
- IMPORTANT: Do NOT run a full migration during feature development.
  - Deliver TypeScript ETL steps per feature under `migration/steps/*`.
  - Final data migration executes once at cut‑over.
