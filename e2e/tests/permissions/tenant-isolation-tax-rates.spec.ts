...
test.describe('Tax Rates Tenant Isolation', () => {
  test('tax rates are strictly isolated between tenants @permissions @taxRates @isolation', async ({ isMobile, page, permissionOverride }) => {
  ...
  test('imported tax rates cannot be accessed cross-tenant @permissions @taxRates @isolation', async ({ isMobile, page, permissionOverride }) => {
    await permissionOverride({
      roleName: 'Admin',
      add: ['admin:manageTaxes'],
      remove: [],
  ...
  test('tax rate selection in templates respects tenant isolation @permissions @taxRates @isolation', async ({ isMobile, page, permissionOverride }, testInfo) => {
    test.fixme(
    ...
    const createTemplateLink = page.getByRole('link', { name: 'Create template' }).first();
    const createTemplateButton = page.getByRole('button', { name: 'Create template' }).first();
  ...
  test('API endpoints enforce tenant scoping @permissions @taxRates @isolation', async ({ isMobile, page, permissionOverride }) => {
    await permissionOverride({
      roleName: 'Admin',
      add: ['admin:manageTaxes'],
      remove: [],
  ...
  test('no enumeration of other tenant tax rates possible @permissions @taxRates @isolation', async ({ isMobile, page, permissionOverride }) => {
    await permissionOverride({
      roleName: 'Admin',
      add: ['admin:manageTaxes'],
      remove: [],
