...
test.describe('Documentation: Discount provider journey — admin setup', () => {
  ...
  test('Admin enables ESN discount provider', async ({ page, tenant }, testInfo) => {
    ...
    const providerToggle = page.getByTestId('enable-esn-provider').getByRole('switch');
    ...
    await takeScreenshot(testInfo, page, page, 'Admin enables ESN provider');
...
test.describe('Documentation: Discount provider journey — member experience', () => {
  test.use({ storageState: userStateFile });

  test('Member registers an ESN discount card', async ({ page, tenant }, testInfo) => {
    test.skip(true, 'ESN card validation requires reliable upstream test numbers.');

    await page.goto('/profile/discount-cards', { waitUntil: 'domcontentloaded' });
    await expect(page.locator(CTA_SECTION)).toBeVisible();
    await takeScreenshot(testInfo, page.locator(CTA_SECTION), page, 'CTA encourages ESN registration');
    ...
    await takeScreenshot(testInfo, cardPanel, page, 'Verified ESN card on file');
