...
The event-list input no longer contains `userId`, and `EventListService` no
longer depends on `users.maybeSelf`. User-specific filtering comes from the
trusted RPC request context. Preserve that boundary while measuring the current
infinite-query lifecycle.

Use RPC, service, and Playwright evidence to verify one initial event-list
request per filter state. Investigate any remaining duplicate method/input pairs
without restoring a user-supplied identity field.
...
Authenticated context resolution no longer queries the `user_attributes`
view or emits `Server.resolveUserAttributes`. Measure the remaining tenant,
user-assignment and onboarding reads with their current query shapes.

Load additional account information only where it is needed. Keep authorization
fail-closed: missing required context may not silently become permission.
...
- `event_registration_options` already has an index led by `event_id`;
...
- whether the existing `event_registration_options(event_id)` index is used;
...
1. record the before plan, execution time, rows, loops, shared-buffer hits, and
   reads;
2. add it through the Drizzle schema and the documented schema release process;
