...
Stripe tax-rate information always belongs to a configured payment account.
The target schema enforces that shape directly, and server writers lock the
organization before reading its account. Only the first account attachment is
supported; changing or removing an attached account is unavailable.
The separate old-to-new data transfer must verify every legacy tax rate with
the provider and record its exact account ownership. Nullable staging rows and
production backfills are not part of the fresh-schema release.
