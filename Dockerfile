...
RUN --mount=type=secret,id=FONT_AWESOME_TOKEN yarn config set npmScopes.fortawesome.npmAuthToken \${FONT_AWESOME_TOKEN}
RUN --mount=type=secret,id=FONT_AWESOME_TOKEN yarn install --immutable
COPY --chown=appuser:appuser . .
RUN --mount=type=secret,id=FONT_AWESOME_TOKEN yarn build
USER root
