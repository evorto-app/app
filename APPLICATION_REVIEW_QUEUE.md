...
| RQ-015 | Final verification          | Lint, format, build, unit, integration, Playwright, Browser acceptance            | Complete | App/spec type checks, format, lint, build, 945 Angular tests, and 1,485 server tests passed; PostgreSQL, E2E, and Browser environment blocks are recorded without substitute claims |
...
| F-049 | Medium   | Candidate indexes are inferred rather than measured                                                                                | Confirm with representative `EXPLAIN`; add only proven indexes                | Accepted: isolated PostgreSQL could not start; static review added no speculative index |
...
| C-008 | Huge event/registration handlers                           | 5,000-line handler and 3,800-line service mix eligibility, checkout, check-in, transfer, and query policy                                          | Extract only stable domain operations at existing transaction seams; keep orchestration local                                                                  | Worth exploring | Resolved: extracted one shared immutable price-snapshot read boundary                               |
...
- Test discovery, skip inventory, storage-state, and suite-ownership tests:
  29/29; baseline Playwright collection: 150 tests in 44 files.
...
- Integrated Angular suite: 124 files, 945 tests passed.
- Integrated server suite: 192 files, 1,485 tests passed.
- `bun run test:e2e:check` failed at environment preflight for the explicitly
  listed missing secrets; no test scenario executed.
- PostgreSQL integration, representative `EXPLAIN`, and interactive Browser
  acceptance are recorded above as environment-blocked, not passed.
