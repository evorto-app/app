...
Current totals: **155 findings** — 1 Critical, 53 High, 87 Medium, and 14
Low. **153 are resolved and 2 are accepted with explicit boundaries. No finding
awaits a product decision.**
...
| D-008 | Shared payment accounts                  | One payment account may intentionally serve multiple Evorto organizations. No cross-organization uniqueness enforcement is required. Each organization's members, events, settings, access, and payment records remain isolated.                                                                                                                                                                                                                                                                                                                            | Accepted: the existing attach-once operation remains scoped to one organization while allowing the same account elsewhere. No uniqueness constraint, cross-organization lookup, or compatibility behavior is added.                                                                                      |
...
- 2026-08-05: Settled D-008 by explicitly allowing one payment account to serve
  multiple organizations. Each attachment and every payment action remains
  organization-scoped; no cross-organization uniqueness, lookup, access, or
  compatibility path was added.
- 2026-08-05: Recovered the two ESNcard examples and Google Maps value already
  present in the primary checkout. The test commands now fill only those exact
  missing names in memory, never print or copy their values, and never import
  database, authentication, port, or other checkout settings. Missing values
  still stop the normal preflight visibly.
- 2026-08-05: The complete publication run surfaced a stale Google Maps guide
  name and one premature role-page capture. Corrected the catalog directly and
  waited for the role page to finish becoming interactive; no alias, retry,
  timeout increase, or fallback was added. The official rerun passed 54/54,
  published all 13 guides and 199 assets, and removed all five retired guide
  destinations.
- 2026-08-05: The final live ESNcard run exposed a stale anchored status
  matcher and stale empty-state copy in the functional journey; the application
  had rendered the verified card correctly. Removed the unnecessary cache
  experiment, then made the journey wait for hydration, poll the persisted
  status, and prove that `lastCheckedAt` advances. The release command passed 8
  focused unit checks and all 9 provider-backed Playwright journeys without a
  retry, delay, or product fallback.
- 2026-08-05: The complete functional baseline surfaced F-155: the tax-rate
  import action appeared enabled in server-rendered output before the page was
  ready to handle its click. The journey now waits for the existing readiness
  marker to clear and clicks once. No second click, delay, weakened assertion,
  or product fallback was added; the full rerun passed 153/153.
- 2026-08-05: Confirmed the local Docker lifecycle boundary. The persistent
  worktree stack and Playwright's disposable stack cannot own the same project
  at once. The reliable sequence is to stop the persistent stack before
  Playwright and recreate it before PostgreSQL verification. The first final
  PostgreSQL command received `ECONNREFUSED` after Playwright intentionally
  removed its stack; an explicit `docker:start` followed by the unchanged
  command passed all 17 files and 83 tests. This is an operating-order fix, not
  an application-code fix, and the unrelated `evorto-cda27261` project remained
  untouched.
- 2026-08-05: The first final image build overlapped other heavy application and
  Docker builds, exhausted Docker's available memory, and ended with exit 137.
  It produced no usable image-security result. No code fix or fallback is
  claimed. After the heavy gates were serialized, the isolated unchanged rerun
  built the 95,726,857-byte linux/amd64 image, excluded runtime source maps,
  exported them separately, and found zero vulnerabilities.
- 2026-08-05: Closed the exact-current-tree gate: the frozen local install made
  no changes; format, lint, 144-file/1,106-test Angular, 199-file/1,603-test
  server, build, release validation, 11/11 integration journeys, and Terraform
  bootstrap, production, and staging with zero misconfigurations pass. The
  provider-backed publication passed 54/54 and produced 13 guides with 199
  assets. The live ESNcard release check passed 8 focused unit checks and all 9
  journeys.
- 2026-08-05: Final visual inspection surfaced F-156 in the first-steps guide:
  an email caption accompanied a full-page image that also exposed the password
  form and internal test-environment branding. The source journey now requests
  an optional exact-element crop, asserts that `person@example.org` is visible
  and editable before one capture, then restores the real one-time account. No
  generated-page patch, retry, fallback, or application behavior change was
  added. The corrected official publication passed 54/54 and copied 13 guides
  with 199 assets. The exact neutral crop was visually inspected; Pages tooling
  passed 19/19 with lint, `git diff --check`, and the 23-page build.
...
The current branch meets the implementation, review, publication, and local
verification requirements. All 15 coverage areas are complete, all 155
findings are resolved or explicitly accepted, and all eight product decisions
are settled. No external prerequisite is hidden by fallback, compatibility
behavior, or weaker evidence.
