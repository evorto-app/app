...
```ts
const result = loadUser("u_123").pipe(Effect.tap((user) => Effect.logDebug("loaded user", { userId: user.id })));
```
...
```ts
const loadUser = (userId: string) => Effect.service(UserRepo).pipe(Effect.flatMap((repo) => repo.getById(userId)));
```
