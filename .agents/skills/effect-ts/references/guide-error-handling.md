...
const decodeUser = (input: unknown) => Schema.decodeUnknownEffect(UserPayload)(input).pipe(Effect.catchTag("SchemaError", (error) => Effect.fail(new InvalidRequestBody({ message: error.message }))));
```
...
```ts
const recovered = program.pipe(Effect.catchTag("UserNotFound", (error) => Effect.succeed({ id: error.userId, guest: true })));
```
...
const program = longRunningTask.pipe(Effect.onInterrupt(() => Console.log("cleaning up after interrupt")));
```
