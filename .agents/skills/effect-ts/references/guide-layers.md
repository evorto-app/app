...
const AppLayer = Layer.mergeAll(UserLayer, BillingLayer, HttpLayer).pipe(Layer.provide(Telemetry), Layer.provide(NodeSdk));
```
...
```ts
const AppLayer = Layer.provide(Layer.mergeAll(Layer.provide(UserRepoLayer, Layer.mergeAll(ConfigLayer, LoggerLayer)), Layer.provide(BillingServiceLayer, Layer.mergeAll(ConfigLayer, LoggerLayer, DatabaseLayer)), HttpLayer), Telemetry).pipe(Layer.provide(NodeSdk));
```
