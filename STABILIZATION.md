...
- `tests/docs/events/register.doc.ts` documents that the ticket QR code is available after registration/payment, no longer claims QR email delivery exists in the current relaunch flow, and now explains the queued confirmation, cancellation, transfer-completed, and waitlist spot-available registration lifecycle emails.
