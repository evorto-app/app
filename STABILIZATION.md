...
Latest coverage checkpoint: generated-doc raw Markdown image source coverage now
explicitly pins optional-call attach-helper invocations before generated docs
can bypass the shared screenshot helper. The guard covers
...
validation for the slice passed `bun run format:write`,
`bun run lint`,
...
PR checks are tracked separately; the latest fully green E2E evidence remains
