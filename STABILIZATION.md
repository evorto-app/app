...
Latest coverage checkpoint: PR head `1eaa2789` has fresh in-app Browser public
General evidence before the final CI/docs checkpoint. A forced Docker Desktop
backend restart recovered the local container start path, `bun run docker:start`
rebuilt and reseeded the generated Docker stack, `bun run dev:status` passed at
`BASE_URL=http://localhost:4218`, and
...
current-head E2E run `27103901612` has green `functional-1` and `functional-2`
shards while the docs shard is still in progress after this local Browser
checkpoint.
...
The current pushed-head public General Browser refresh on PR head `1eaa2789`
kept the evidence current after the generated-doc evidence stabilization push.
A forced Docker Desktop backend restart recovered the local container start
path; `bun run docker:start` rebuilt the generated Docker app, `bun run
dev:status` passed at `BASE_URL=http://localhost:4218`, and
`bun run test:e2e:public-general-viewports` passed both local General viewport
tests. The localhost Browser evidence shifted only local seeded demo event dates
