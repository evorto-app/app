...
- `tests/docs/events/unlisted-user.doc.ts` covers the participant-facing
  ...
  its direct detail URL, screenshotting the registration section with a visible
...
- Current Neon active-test branch cleanup checkpoint: the attached Neon Local
  ...
  uses `PARENT_BRANCH_ID` or resolves the default parent branch so Neon Local
  creates an ephemeral branch, and persistent branch modes remain explicit local
  opt-ins that cleanup intentionally skips. Recent Neon operations still showed the GitHub
