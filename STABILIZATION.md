...
Latest coverage checkpoint: generated-doc raw Markdown image detection now
tracks parameter-destructured markdown names, raw Markdown bodies, raw Markdown
payloads, and attach helpers from constant-backed computed properties before
rejecting unsupported image markup. The guard covers grouped metadata with
`[markdownNameKey]: 'markdown'`, `[rawBodyKey]: '![raw](raw.png)'`,
`[rawPayloadKey]: { body: '<img src="../raw.png" alt="Raw">' }`, and
`[attachKey]: testInfo.attach.bind(testInfo)`, then follows parameter
destructuring such as `[markdownNameKey]: markdownName`,
`[rawBodyKey]: rawMarkdownBody`, `[rawPayloadKey]: rawMarkdownPayload`, and
`[attachKey]: attachMarkdown` before rejecting
`testInfo.attach(markdownName, { body: rawMarkdownBody })`,
`testInfo.attach('markdown', rawMarkdownPayload)`, and
`attachMarkdown(markdownName, rawMarkdownPayload)`. A docs test can no longer
hide unrelated or uncaptioned raw Markdown imagery by moving the attachment
name, body, payload, or helper into a computed-key parameter object. Local
validation for the slice passed `bun run format:write`, `bun run lint`,
...
with 204 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
