...
Latest coverage checkpoint: generated-doc screenshot-density detection now tracks
parameter-destructured Markdown names and attach helpers from constant-backed
computed properties before enforcing the
two-screenshots-per-explanatory-section limit. The guard covers grouped metadata
with `[markdownNameKey]: 'markdown'` and
