...
Latest coverage checkpoint: optional-call Playwright modifier/page helper
invocations are now normalized before the skip/focus/runtime/debug/fixed-wait
inventory compares source against its allowlists. The guard covers
`test.skip?.(...)`, `test?.fixme(...)`,
`test.describe?.[onlyKey]?.(...)`, `test.slow?.()`, `hiddenSlow?.()`,
`page.pause?.(...)`, `page?.[pauseKey]?.()`, `page.waitForTimeout?.(...)`,
`page[waitKey]?.(...)`, and `waitByAlias?.(...)`. A future spec or docs test
cannot hide skipped, focused, serial/slow, interactive debug, or fixed-wait
coverage changes by using optional chaining on direct helpers, constant-backed
properties, or local aliases. Local validation for the slice passed
...
with 91 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
