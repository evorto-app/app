...
evidence was captured on PR head `7cc6dcc1` after reconnecting to the already
running worktree Docker app at `BASE_URL=http://localhost:4200`. `bun run
dev:status` confirmed the app route probe passed for `/legal/terms`, that no
other Evorto Compose project owned port 4200, and that the database validation
query succeeded; the Docker disposable-container start-path preflight still
timed out, so this checkpoint uses the existing healthy app rather than a fresh
container start. Browser rechecked the anonymous General route set at 390x844
and 320x740: `/`, `/events`, `/legal/imprint`, `/legal/privacy`,
...
checks found expected current content after accounting for current route copy
(`/` redirects to Events, `/403` says "Access not allowed", and event details
use "Registration"), no persistent loading text, no application-error text, no
true horizontal overflow or visible overflow entries, no side/top clipped
visible controls, fixed Events/Login mobile navigation, and zero Browser
...
copy, readable Terms fallback copy, the not-found page, and fixed Events/Login
mobile navigation without visible overlap. The prior PR-head `1eaa2789` sweep
remains the latest full 320x740/390x844/1440x900 desktop-and-mobile Browser
matrix; the live PR checks are tracked separately because evidence-recording
pushes can supersede in-progress E2E runs without changing the last fully green
CI baseline.
...
- `tests/specs/scanning/scanner.test.ts` now creates deterministic confirmed
  ...
  checked-in aggregate shown there before restoring the seeded counter. It also
  moves the seeded scanner event into the real one-hour pre-start check-in
  window for each page-backed scanner path, then restores the original event
  start/end times so CI timing drift cannot disable the check-in controls.
...
- Template tax-rate fixme cleanup pass: removed the future bulk/no-compatible-rate
  ...
  there is no bulk UI surface. Later page-backed coverage now opens the paid
  template tax-rate selector and pins the no-compatible-rate select feedback,
  while local component coverage still pins loading, failed, empty, and
  available helper states.
...
- Current generated-docs evidence-quality guard checkpoint:
  ...
  runtime rejection for raw image markup outside the shared screenshot helper.
  ...
  `docs-baseline` suite without starting the webserver, and reported `32 tests`
...
`{ ['shell']: page.locator('main') }`, plus constant-backed computed keys such
as `{ [shellKey]: page.locator('main') }` and `groupedTargets[shellKey]`, so
bracketed grouped targets do not bypass meaningful-image enforcement. It also
resolves `Reflect.get(...)` reads of grouped generic, broad, single-control, and
icon/media target properties, including local `Reflect` aliases and
constant-backed keys, so reflected weak target lookups cannot bypass
meaningful-image enforcement. Constant-backed selector, role, and test-id
values now feed the same generic, broad,
...
covered as well, so `const spreadTest = { ...test }`,
`const assignedDescribe = Object.assign({}, test.describe)`,
`const spreadPage = { ...page }`, and
`const assignedPage = Object.assign({}, page)` cannot copy the helper object
into a local alias that bypasses the inventory. Copied-object
`spreadTest.skip(...)`, `spreadTest[skipKey](...)`,
`assignedDescribe.only(...)`, `assignedDescribe[configureKey](...)`,
`spreadPage.pause(...)`, and `assignedPage[waitKey].apply(...)` are classified
as skip/focus/runtime, interactive debug, or fixed-wait entries before
...
`bind(...)(...)`, and `Reflect.apply(...)` invocations of real Markdown
attachment helpers, so `testInfo.attach.call(testInfo, markdownName, ...)` and
`attachMarkdown.bind(testInfo)(markdownName, ...)` cannot publish a thin
...
into the future so Browser could inspect public event cards without test-cookie
injection. The refined in-app Browser sweep checked all 10 anonymous General
routes at 320x740, 390x844, and 1440x900: `/`, `/events`, seeded event detail
...
correct Events/Login mobile navigation with Scanner hidden, correct
Events/Scanner/Login desktop navigation, and zero Browser warning/error logs.
...
meaningful seeded Material event cards with icons/times and fixed bottom
navigation, readable Terms fallback copy, readable not-found fallback copy, and
the desktop list/detail event registration surface with side navigation rather
...
Object-spread and `Object.assign` copied Playwright modifier/page helper objects
are now classified beside object-rest copied helpers before the
skip/focus/runtime/debug/fixed-wait inventory compares source against its
allowlists. The guard covers `const spreadTest = { ...test }`,
`const assignedDescribe = Object.assign({}, test.describe)`,
`const spreadPage = { ...page }`, and
`const assignedPage = Object.assign({}, page)`, including direct calls and
constant-backed bracket access such as `spreadTest[skipKey](...)`,
`assignedDescribe[configureKey](...)`, and `assignedPage[waitKey].apply(...)`.
A future spec or docs test cannot hide skipped, focused, serial/slow,
interactive debug, or fixed-wait coverage changes by copying the helper object
through spread or `Object.assign`. The test inventory and stabilization source
guard pin this copied-object coverage beside the object-rest, direct,
bracket-property, local-alias, nested-destructuring, static-key,
array-built-key, and reflection-based modifier/page-helper cases. Validation for
this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/playwright-skip-inventory.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 90 tests, and `git diff --check`; WebStorm errors-only diagnostics
remained blocked because this worktree is not one of the IDE's open projects.
Browser verification was attempted, but no positive visual evidence is claimed
for this source-only slice: `bun run dev:status` found the generated database
endpoint accepting TCP connections but failing the validation query with
`Connection terminated unexpectedly`, Docker's disposable start-path preflight
timed out, the project-scoped `db` container was unhealthy, and the existing
app route probe could not connect to `/legal/terms`. The in-app Browser probe
at 390x844 tried
...
Optional-call Playwright modifier/page helper invocations are now normalized
before the skip/focus/runtime/debug/fixed-wait inventory compares source
against its allowlists. The guard covers `test.skip?.(...)`,
`test?.fixme(...)`, `test.describe?.[onlyKey]?.(...)`, `test.slow?.()`,
`hiddenSlow?.()`, `page.pause?.(...)`, `page?.[pauseKey]?.()`,
`page.waitForTimeout?.(...)`, `page[waitKey]?.(...)`, and
`waitByAlias?.(...)`, including direct helpers, constant-backed properties, and
local aliases. A future spec or docs test cannot hide skipped, focused,
serial/slow, interactive debug, or fixed-wait coverage changes by using
optional chaining. The test inventory and stabilization source guard pin this
optional-call coverage beside the direct, bracket-property, local-alias,
nested-destructuring, static-key, array-built-key, reflection-based,
object-rest, and copied-object modifier/page-helper cases. Validation for this
slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/playwright-skip-inventory.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 91 tests, and `git diff --check`; WebStorm errors-only diagnostics
remained blocked because this worktree is not one of the IDE's open projects.
Browser verification was attempted, but no positive visual evidence is claimed
for this source-only slice: `bun run dev:status` found the generated database
endpoint accepting TCP connections but failing the validation query with
...
Generated-doc weak screenshot target detection now resolves constant-backed
computed grouped target keys before target-quality checks run. The guard covers
grouped objects such as `{ [shellKey]: page.locator('main') }` and corresponding
reads such as `groupedTargets[shellKey]`, `groupedTargets[broadKey]`,
...
targets behind computed object keys. The implementation now passes the shared
static-string alias map into grouped weak-target alias collection and computed
assignment references, so literal and constant-backed computed properties are
handled by the same generated-doc image-quality source guards. Validation for
this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 200 tests, and `git diff --check`; WebStorm errors-only diagnostics
remained blocked because this worktree is not one of the IDE's open projects.
Browser verification was attempted, but no positive visual evidence is claimed
for this source-only slice: `bun run dev:status` found the generated database
endpoint accepting TCP connections but failing the validation query with
...
endpoint accepting TCP connections but failing the validation query with
`Connection terminated unexpectedly`, Docker's disposable start-path preflight
passed, and the existing app route probe still could not connect to
`/legal/terms`. The in-app Browser probe at 390x844 tried
`/legal/terms?stabilizationEvidence=computed-destructured-target-source-guard`
and failed before rendering the page with `net::ERR_BLOCKED_BY_CLIENT`.

Generated-doc markdown attachment counting now resolves parameter-destructured
markdown names from constant-backed computed properties before the manifest
guard decides whether a docs file has enough explanatory sections. The guard
covers grouped metadata with `[markdownNameKey]: 'markdown'`, parameter
destructuring with `[markdownNameKey]: markdownName`, and the paired
`[attachKey]: attachMarkdown` helper alias, then counts both
`testInfo.attach(markdownName, ...)` and
`attachMarkdown(markdownName, ...)` as real Playwright-backed Markdown
sections. The implementation now collects destructured parameter aliases for
`markdownAttachmentNamePropertyAliases` in the same generated-doc markdown-count
path that already collected destructured attach-helper aliases, so constant
computed names are resolved consistently for variables and function
parameters. Validation for this slice passed `bun run format:write`,
`bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 202 tests, and `git diff --check`; WebStorm errors-only diagnostics
remained blocked because this worktree is not one of the IDE's open projects.
Browser verification was attempted for this source-only slice, but no positive
visual evidence is claimed: `bun run dev:status` found the generated database
endpoint accepting TCP connections but failing the validation query with
...
Generated-doc weak Markdown body detection now resolves parameter-destructured
markdown names and attach helpers from constant-backed computed properties
before enforcing the per-section 120-character explanatory body minimum. The
guard covers grouped metadata with `[markdownNameKey]: 'markdown'`, parameter
destructuring with `[markdownNameKey]: markdownName`, and the paired
`[attachKey]: attachMarkdown` helper alias, then rejects both
`testInfo.attach(markdownName, { body: 'Too short.' })` and
`attachMarkdown(markdownName, { body: 'Also short.' })`. The implementation now
collects parameter initializer aliases, direct `attach` parameter bindings, and
destructured parameter property aliases in the weak-body source guard, matching
the generated-doc markdown-count path so thin prose cannot bypass body-quality
checks through helper parameters. Validation for this slice passed
`bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 203 tests, and `git diff --check`; WebStorm errors-only diagnostics
remained blocked because this worktree is not one of the IDE's open projects.
Browser verification was attempted for this source-only slice, but no positive
visual evidence is claimed: `bun run dev:status` found the generated database
endpoint accepting TCP connections but failing the validation query with
...
Generated-doc raw Markdown image detection now resolves parameter-destructured
markdown names, raw Markdown bodies, raw Markdown payloads, and attach helpers
from constant-backed computed properties before rejecting unsupported image
markup. The guard covers grouped metadata with `[markdownNameKey]: 'markdown'`,
`[rawBodyKey]: '![raw](raw.png)'`,
`[rawPayloadKey]: { body: '<img src="../raw.png" alt="Raw">' }`, and
`[attachKey]: testInfo.attach.bind(testInfo)`, then follows parameter
destructuring with `[markdownNameKey]: markdownName`,
`[rawBodyKey]: rawMarkdownBody`, `[rawPayloadKey]: rawMarkdownPayload`, and
`[attachKey]: attachMarkdown`. The implementation now collects destructured
parameter property aliases for markdown attachment names, raw Markdown bodies,
raw Markdown payloads, and attach helpers in the raw-image source guard, so a
docs test cannot hide unrelated or uncaptioned imagery by moving any part of
the Markdown attachment behind a computed-key parameter object. Validation for
this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 204 tests, and `git diff --check`; WebStorm errors-only diagnostics
remained blocked because this worktree is not one of the IDE's open projects.
Browser verification was attempted for this source-only slice, but no positive
visual evidence is claimed: `bun run dev:status` found the generated database
endpoint accepting TCP connections but failing the validation query with
...
Generated-doc weak screenshot target detection now resolves
parameter-destructured generic shell, broad section, single-control, and
icon/media targets from constant-backed computed properties before
meaningful-target checks run. The guard covers grouped targets with
`[shellKey]: page.locator('main')`, `[broadKey]: page.locator('section')`,
`[singleKey]: page.getByRole('button', { name: 'Save' })`, and
`[iconKey]: page.locator('svg')`, then follows parameter destructuring with
`[shellKey]: shellTarget`, `[broadKey]: broadTarget`,
`[singleKey]: singleTarget`, and `[iconKey]: iconTarget`. The implementation
now collects destructured parameter property aliases in the generic, broad,
single-control, and icon/media target detectors, so a docs test cannot hide a
weak screenshot crop by moving the target behind a computed-key parameter
object. Validation for this slice passed `bun run format:write`,
`bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 205 tests, and `git diff --check`; WebStorm errors-only diagnostics
remained blocked because this worktree is not one of the IDE's open projects.
Browser verification was attempted for this source-only slice, but no positive
visual evidence is claimed: `bun run dev:status` found the generated database
endpoint accepting TCP connections but failing the validation query with
...
Generated-doc screenshot-density detection now tracks parameter-destructured
Markdown names and attach helpers from constant-backed computed properties
before enforcing the two-screenshots-per-explanatory-section limit. The guard
covers grouped metadata with `[markdownNameKey]: 'markdown'` and
`[attachKey]: testInfo.attach.bind(testInfo)`, then follows parameter
destructuring such as `[markdownNameKey]: markdownName` and
`[attachKey]: attachMarkdown`. It accepts the first two screenshots after each
computed-parameter Markdown section but rejects the third screenshots at lines
29 and 52, so generated docs cannot hide screenshot clusters without nearby
explanatory prose by moving Markdown attachment names or helpers into
computed-key parameter objects. Local validation for the slice passed
`bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 206 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
blocked because this worktree is not one of the IDE's open projects. Browser
...
Generated-doc screenshot-density source coverage now explicitly pins
optional-call Markdown attachment syntax before enforcing the
two-screenshots-per-explanatory-section limit. The guard covers
`testInfo.attach?.(markdownName, ...)` and
`attachMarkdown?.(markdownName, ...)`, accepts the first two screenshots after
each optional-call Markdown section, and rejects the third screenshots at lines
21 and 44. Generated docs cannot hide screenshot clusters without nearby
explanatory prose by switching explanatory Markdown calls to optional-call
syntax. Local validation for the slice passed `bun run format:write`,
`bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 207 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
blocked because this worktree is not one of the IDE's open projects. Browser
verification was attempted, but no positive visual evidence is claimed for this
source-only slice: `bun run dev:status` found the generated database endpoint
accepting TCP connections but failing the validation query with
...
Generated-doc weak Markdown body source coverage now explicitly pins
optional-call Markdown attachment syntax before enforcing the 120-character
explanatory body minimum. The guard covers
`testInfo.attach?.(markdownName, ...)` and
`attachMarkdown?.(markdownName, ...)`, and rejects the optional-call short
bodies at lines 4 and 5 while ignoring a non-Playwright
`recordDocumentationNote?.(...)` call. Generated docs cannot hide thin
explanatory prose by switching Markdown attachments to optional-call syntax.
Local validation for this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 208 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
blocked because this worktree is not one of the IDE's open projects. Browser
verification was attempted, but no positive visual evidence is claimed for this
source-only slice: `bun run dev:status` found the generated database endpoint
accepting TCP connections but failing the validation query with
...
Generated-doc Markdown-count source coverage now explicitly pins optional-call
Markdown attachment syntax before the manifest guard compares expected
explanatory sections. The guard covers
`testInfo.attach?.(markdownName, ...)` and
`attachMarkdown?.(markdownName, ...)`, counts both optional-call Markdown
sections, and ignores a non-Playwright `recordDocumentationNote?.(...)` call.
Generated docs cannot silently lose explanatory sections from the manifest
guard by switching Markdown attachments to optional-call syntax. Local
validation for this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 209 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
blocked because this worktree is not one of the IDE's open projects. Browser
verification was attempted, but no positive visual evidence is claimed for this
source-only slice: `bun run dev:status` found the generated database endpoint
accepting TCP connections but failing the validation query with
...
Generated-doc raw Markdown image source coverage now explicitly pins
optional-call attach-helper invocations before generated docs can bypass the
shared screenshot helper. The guard covers
`attachMarkdown?.(markdownName, ...)` and
`helperGroup[attachKey]?.(markdownName, ...)`, rejects both a raw Markdown image
body and a raw HTML image body, and ignores a non-Playwright
`recordDocumentationNote?.(...)` call. Generated docs cannot embed unrelated
images by switching a tracked attach helper to optional-call syntax. Local
validation for this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
...
Public General viewport coverage now checks fixed Events/Login navigation label
readability on painted Material surfaces in the same light/dark readability
matrix as the hosted legal and error pages. The Playwright spec now lets the
shared contrast helper evaluate either CSS selectors or locators, applies it to
the visible Events and Login navigation labels, and calls the anonymous
navigation assertion inside the light/dark simple-page loop. General pages
cannot keep readable headings while the fixed mobile navigation silently
regresses to low-contrast text. Local validation for this slice passed
`bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 210 tests, `bun run test:e2e:public-general-viewports -- --list`, and
`git diff --check`; WebStorm errors-only diagnostics remain blocked because
this worktree is not one of the IDE's open projects. Browser verification was
attempted, but no positive visual evidence is claimed for this source/spec
slice: `bun run dev:status` found the generated database endpoint accepting TCP
connections but failing the validation query with `Connection terminated
unexpectedly`; Docker's disposable start-path preflight passed, but the
existing app route probe still could not connect to `/legal/terms`. The in-app
Browser probe at 390x844 tried
`/legal/terms?stabilizationEvidence=general-navigation-contrast-source-guard`
and failed before rendering the page with `net::ERR_BLOCKED_BY_CLIENT`.

Public General viewport coverage now checks the desktop-only Scanner navigation
label on painted Material surfaces with the same shared contrast helper as
Events and Login. The Playwright spec keeps the mobile branch asserting Scanner
is hidden, then applies the contrast check in the desktop branch where Scanner
is visible. General pages cannot pass mobile navigation coverage while the
desktop side navigation keeps a low-contrast Scanner entry. Local validation
for this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 210 tests, `bun run test:e2e:public-general-viewports -- --list`, and
`git diff --check`; WebStorm errors-only diagnostics remain blocked because
...
existing app route probe still could not connect to `/legal/terms`. The in-app
Browser probe at 390x844 tried
`/legal/terms?stabilizationEvidence=general-scanner-contrast-source-guard` and
failed before rendering the page with `net::ERR_BLOCKED_BY_CLIENT`.

Current-head local runtime and Browser evidence are positive again after
resetting the stale Docker stack. `bun run docker:start` rebuilt the app,
recreated the worktree-local Neon Local branch, and started the generated
`BASE_URL=http://localhost:4218`; `bun run dev:status` then passed with a
healthy database validation query, a passing `/legal/terms` route probe, and
only the fresh active-test Neon branch inside the two-hour TTL.
`bun run test:e2e:public-general-viewports -- --no-deps` passed both General
viewport tests against the running app. The in-app Browser checked the full
anonymous General route set at 320x740, 390x844, and 1440x900 on current head,
including root redirect, events list, seeded event detail, hosted legal pages,
403, 500, explicit 404, and wildcard 404. The Browser pass found expected
content, no visible clipped controls, no persistent loading labels, no
application-error text, fixed Events/Login mobile navigation, fixed
Events/Scanner/Login desktop navigation, and zero warning/error Browser logs.
Representative screenshots were inspected at
...
`/tmp/evorto-current-general-browser-evidence/event-detail-1440x900.png`; they
show seeded event cards, legal fallback copy, desktop side navigation, and the
selected event detail registration surface rather than unrelated or heading-only
images. A Browser-only documentElement scroll-width reading on simple legal and
error pages included the browser scrollbar gutter, but follow-up inspection
...
Pushed checkpoint `7fcbeecd` made the finance and profile/account Docker-backed
runtime evidence reviewable from the test inventory instead of only from the
stabilization narrative. `tests/test-inventory.md` names the
exact finance specs/docs that passed in the Docker-backed system-Chrome run,
including finance overview permissions, receipt flows, Stripe webhook replay,
inclusive tax-rate import, and the three finance generated-doc journeys. It
also names the exact profile/account specs/docs that passed in the
Docker-backed system-Chrome run, including profile edit, event cards, receipts,
discounts, deterministic ESNcard provider outcomes, and the profile
generated-doc journeys. The stabilization source guard now pins those file
lists to both the inventory and this document so the runtime evidence cannot
stay broad while the discoverable inventory loses the covered paths. Focused
validation for this slice passed `bun run format:write`, `bun run lint`,
`bunx vitest run helpers/testing/generated-documentation-source.spec.ts helpers/testing/stabilization-source.spec.ts --reporter=verbose`
with 210 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
blocked because this worktree is not one of the IDE's open projects. The
same-slice in-app Browser mobile check opened
`/legal/terms?stabilizationEvidence=finance-profile-inventory-guard` at
390x844 on the running Docker app. The inspected screenshot
`/tmp/evorto-finance-profile-inventory-guard-terms-390x844.png` shows readable
Terms fallback copy and fixed Events/Login bottom navigation rather than an
unrelated or heading-only artifact; Browser metrics found no clipped controls,
Scanner hidden on mobile, zero warning/error logs, no loading text, and no
...
validation query and `/legal/terms` route probe and confirmed no other Evorto
Compose project owned port 4200; the Docker disposable-container start-path
preflight still timed out, so this evidence uses the existing healthy app
instead of claiming a fresh container start. Browser rechecked `/`, `/events`,
`/legal/imprint`, `/legal/privacy`, `/legal/terms`, `/403`, `/500`, `/404`,
`/missing-general-page`, and seeded event detail
`/events/033d11cabd68960b7d61` at 390x844 and 320x740. All 20 mobile
route/viewport checks found expected current route copy, no persistent loading
text, no application-error text, no true horizontal overflow or visible
overflow entries, no side/top clipped visible controls, fixed Events/Login
mobile navigation, and zero Browser warning/error logs. The inspected
screenshots `/tmp/evorto-browser-7cc6dcc1-current-mobile/events-320x740.png`,
`/tmp/evorto-browser-7cc6dcc1-current-mobile/events-033d11cabd68960b7d61-320x740.png`,
`/tmp/evorto-browser-7cc6dcc1-current-mobile/legal-terms-390x844.png`, and
`/tmp/evorto-browser-7cc6dcc1-current-mobile/missing-general-page-390x844.png`
show seeded Material event cards with icons/times, the event detail hero and
copy, readable Terms fallback copy, the not-found page, and fixed Events/Login
mobile navigation without visible overlap.
