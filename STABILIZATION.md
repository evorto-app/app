...
Latest coverage checkpoint: the template paid-registration empty
compatible-tax-rate gap is now covered by a page-backed Playwright case,
generated finance-doc text, and a Material overlay screenshot of the visible "No
active inclusive tax rates available" selector feedback. Local validation for
the slice passed format, lint, the focused generated-doc source and
stabilization source suites with 194 tests,
`bun run test:e2e:docs -- --list tests/docs/finance/inclusive-tax-rates.doc.ts`,
`git diff --check`, and Browser mobile checks for `/legal/terms` plus `/events`
at 390x844. Authenticated focused Playwright verification is still gated by the
unregistered generated Auth0 callback port `localhost:4218`, while the
registered `localhost:4200` port is occupied by a different Compose project.
The live PR checks are tracked separately; the latest fully green E2E evidence
