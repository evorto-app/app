...
Latest pushed-head checkpoint: PR head `d4ea11a9` extends generated-doc
source guards for object-rest copied grouped screenshot-helper aliases and
records Browser evidence from `/legal/terms` plus `/events` at 390x844. Local
validation for the latest slice passed format, lint, the focused generated-doc
source and stabilization source suites with 194 tests, `git diff --check`, and
the Browser screenshots show meaningful Terms fallback and seeded Events card
images with fixed mobile navigation and zero warning/error logs. The PR checks
for `d4ea11a9` are queued/pending; the newer source-guard pushes are not yet
claimed as fully green E2E evidence.
