...
- Tenant admin "General settings" shows a read-only identity summary with tenant name, primary domain, currency, locale, timezone, and Stripe connection state. It lets tenant admins change default location, site theme, SEO title/description, legal links, receipt countries/allow-other, and ESNcard provider enablement plus buy URL. Configured legal links appear in the public app footer.
...
- **Addressed in stabilization pass:** tenant general settings now include a visible deferred-settings summary for custom-domain verification, logo/favicon, hosted legal text pages beyond external links, email sender name, review/publishing settings, registration limits, editable locale/currency/timezone, and Stripe account management. These fields are still not editable because the current tenant schema/update payload does not store them.
