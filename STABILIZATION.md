...
- `tests/docs/events/unlisted-user.doc.ts` covers the participant-facing
  ...
  docs intentionally do not generate admin unlisted-event pages; global-admin
  tenant administration now has its own scoped generated guide.
...
- Observed PR #62 active-head CI cleanup checkpoint: after pushing local head
  `2667494b`, the current PR rollup shows CodeQL, CodeQL `Analyze (actions)`,
...
aliased body variables, aliased payload objects, shorthand `{ body }`, and
bound, destructured, grouped, indexed, or assigned `attach` helpers, so product
docs cannot add unrelated or unhighlighted images outside the shared screenshot
