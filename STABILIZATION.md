...
Latest pushed-head checkpoint: PR head `19268950` closes the template
paid-registration empty compatible-tax-rate coverage gap with a page-backed
Playwright case, generated finance-doc text, and a Material overlay screenshot
of the visible "No active inclusive tax rates available" selector feedback.
Local validation for the latest slice passed format, lint, the focused
generated-doc source and stabilization source suites with 194 tests,
...
The PR checks for `19268950` are queued/pending; the latest fully green E2E
evidence remains the older completed pushed head until the new CI run finishes.
