...
Latest coverage checkpoint: public General viewport coverage now checks the
desktop-only Scanner navigation label on painted Material surfaces with the
same shared contrast helper as Events and Login. The Playwright spec keeps the
mobile branch asserting Scanner is hidden, then applies the contrast check in
the desktop branch where Scanner is visible. General pages cannot pass mobile
navigation coverage while the desktop side navigation keeps a low-contrast
Scanner entry. Local validation for the slice passed `bun run format:write`,
`bun run lint`,
