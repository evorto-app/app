...
Latest coverage checkpoint: object-spread and `Object.assign` copied Playwright
modifier/page helper objects are now classified beside object-rest copied
helpers before the skip/focus/runtime/debug/fixed-wait inventory compares
source against its allowlists. The guard covers `const spreadTest = { ...test }`,
...
through spread or `Object.assign`. Local validation for the slice passed
...
with 90 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
