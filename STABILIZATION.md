...
tracks parameter-destructured generic shell, broad section, single-control, and
...
`[iconKey]: page.locator('svg')`, then follows parameter destructuring such as
`[shellKey]: shellTarget`, `[broadKey]: broadTarget`,
`[singleKey]: singleTarget`, and `[iconKey]: iconTarget` before rejecting
`takeScreenshot(...)` calls that use those weak targets. A docs test can no
longer hide generic page shells, broad sections, button-only crops, or icon-only
images by moving the screenshot target through a computed-key parameter object.
Local validation for the slice passed `bun run format:write`, `bun run lint`,
...
with 205 tests, and `git diff --check`; WebStorm errors-only diagnostics remain
