...
Latest coverage checkpoint: public General viewport coverage now checks fixed
Events/Login navigation label readability on painted Material surfaces in the
same light/dark readability matrix as the hosted legal and error pages. The
Playwright spec now lets the shared contrast helper evaluate either CSS
selectors or locators, applies it to the visible Events and Login navigation
labels, and calls the anonymous navigation assertion inside the light/dark
simple-page loop. General pages cannot keep readable headings while the fixed
mobile navigation silently regresses to low-contrast text. Local validation for
the slice passed `bun run format:write`, `bun run lint`,
