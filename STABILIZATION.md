...
- **Must fix before agent scaling:** several specs silently return or skip when required product state is missing. Remaining examples include event creation setup and scanner preconditions. Receipt approval/refund rows, receipt dialog options, and unlisted-event seed state now fail loudly when expected seeded state is missing.
...
- Convert remaining silent no-op passes in scanner and event-creation specs into explicit fixture setup, hard failures, or honest `test.fixme` states.
